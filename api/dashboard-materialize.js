...
export default async function handler(req, res) {
  try {
    ...
    if (!expected) {
      const sql = getDb();
      const rows = await sql.query("SELECT EXISTS (SELECT 1 FROM public.dashboard_company_snapshot_v01 WHERE contract_version='dashboard_snapshot_v01_2') AS exists");
      if (rows[0]?.exists) {
