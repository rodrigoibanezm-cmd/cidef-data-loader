...
test('episode started before date_from is included and intersection remains auxiliary', () => { const x=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-18'}).history[0]; assert.deepEqual([x.intersection_from,x.intersection_to],['2026-02-15','2026-02-18']); assert.deepEqual([x.vigencia_desde,x.vigencia_hasta],['2026-02-01','2026-02-28']); });
test('episode ending after date_to is included and intersection remains auxiliary', () => { const x=run({version_id:7176,date_from:'2026-02-10',date_to:'2026-02-20'}).history[0]; assert.deepEqual([x.intersection_from,x.intersection_to],['2026-02-10','2026-02-20']); assert.deepEqual([x.vigencia_desde,x.vigencia_hasta],['2026-02-01','2026-02-28']); });
...
test('request-period VIN metrics remain explicit auxiliaries', () => { const x=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-20'}).history[0]; assert.equal(x.vin_facturados_en_periodo,2); assert.deepEqual([x.primer_vin_fecha_en_periodo,x.ultimo_vin_fecha_en_periodo],['2026-02-15','2026-02-20']); });
test('canonical VIN metrics remain full-episode under partial overlap', () => { const x=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-20'}).history[0]; assert.deepEqual([x.vin_facturados,x.primer_vin_fecha,x.ultimo_vin_fecha],[4,'2026-02-05','2026-02-25']); });
test('summary keeps full-episode VIN canonical metric and separate period auxiliary', () => { const r=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-20'}); assert.equal(r.summary.vin_facturados,4); assert.equal(r.summary.vin_facturados_en_periodo,2); });
test('FULL_EPISODE contract survives a filter cutting through the episode', () => {
  const episode = [{ ...base, price_episode_id: 20, vigencia_desde:'2026-02-01', vigencia_hasta:'2026-02-28', tipo_cambio:'INITIAL', source_status:'OK', vin_facturados_en_vigencia:2, primer_vin_fecha:'2026-02-10', ultimo_vin_fecha:'2026-02-20' }];
  const vinRows = [
    { price_episode_id:20, version_id:7176, fecha_factura:'2026-02-10' },
    { price_episode_id:20, version_id:7176, fecha_factura:'2026-02-20' },
  ];
  const r=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-16',include_conflicts:true},episode,vinRows);
  const x=r.history[0];
  assert.equal(r.date_filter_semantics,'EPISODE_OVERLAP');
  assert.equal(r.metrics_scope,'FULL_EPISODE');
  ...
  assert.equal(x.vin_facturados_en_periodo,0);
});
...
test('SOURCE_CONFLICT preserves provenance and has zero VIN', () => { const x=run({version_id:7176,date_from:'2026-03-01',date_to:'2026-03-31'}).history[0]; assert.deepEqual(x.evidence,{source_rows:2,source_variant_count:2,source_files:['a.xlsx']}); assert.equal(x.vin_facturados,0); assert.equal(x.vin_facturados_en_periodo,0); });
...
test('days_active remains full-episode inclusive metadata', () => { const r=run({version_id:7176,date_from:'2026-02-15',date_to:'2026-02-16'}); assert.equal(r.history[0].days_active,28); assert.equal(r.metrics_scope,'FULL_EPISODE'); });
...
test('period without episodes returns valid empty response', () => { const r=run({version_id:7176,date_from:'2025-01-01',date_to:'2025-01-31'}); assert.deepEqual(r.history,[]); assert.equal(r.summary.vin_facturados,0); assert.equal(r.summary.vin_facturados_en_periodo,0); });
