...
function episode(unitId = 7) {
  return {
    unit_id: unitId, unit_label: 'Tienda A', baseline: 'moving_average_12',
...
test('episode evidence exposes ex-ante rows separately from future outcome', () => {
  ...
  const result = buildEpisodeEvidence(rows, [episode()], parsed);
...
test('numeric detail_unit_id matches runtime string unit ids', () => {
  const parsed = parseEpisodeEvidenceInput(input({ detail_unit_id: 7 }));
  const rows = [row('2025-11', 15, 20, -5, 0.2), row('2025-12', 10, 20, -10, 0.1)];
  const result = buildEpisodeEvidence(rows, [episode('7'), episode('8')], parsed);
  assert.equal(result.detail.matched_rows, 1);
  assert.equal(result.detail.returned_rows, 1);
  assert.equal(result.episode_evidence[0].unit_id, '7');
});
