...
function cleanPoint(date, tuple = base, pvid = 1, versionId = 1001) {
  return {
...
function conflictPoint(date, pvid = 1, versionId = 1001) {
  return {
    price_version_id: pvid,
    version_id: versionId,
    resolution_status: 'resolved',
...
test('VIN matching two clean episodes is ambiguous, not arbitrarily assigned', () => {
  ...
  const b = { ...buildPriceEpisodes([cleanPoint('2026-01-01', base, 2, 1001)])[0], vigencia_hasta: null };
  const result = assignVehiclesToEpisodes([
