...
### `current_month_close_forecast_v01`
...
```text
metric: VIN_SALES | SHARE_WITHIN_CIDEF
grain: TOTAL | CHANNEL | STORE | DEALER | DEALER_GROUP | SELLER | BRAND | MODEL | VERSION
filters?: channel, store_id/store, dealer_id/dealer, dealer_group_id/dealer_group,
          seller_id/seller, brand_id/brand, model_id/model, version_id/version
...
No persiste, interpreta, detecta anomalías, genera tesis, aplica materialidad, proyecta ni recomienda acciones.
...
```text
metric: MARKET_SIZE | ENTITY_VIN | MARKET_SHARE | RANK
grain: TOTAL | BRAND | MODEL | SEGMENT | TYPE | REGION | COMUNA | FUEL
universe_filters?: brand_id/brand, model_id/model, segment, type, region, comuna, fuel
entity?: las mismas dimensiones; obligatorio salvo MARKET_SIZE
date_from, date_to: YYYY-MM-DD inclusivos
time_grain: MONTH | YEAR
...
Coverage expone una sola fila `PRODUCT_IDENTITY` con unidades `resolved`, `unresolved`, `ambiguous`, `notApplicable`, `total` y sus ratios. Mide la resolución conjunta por alias de modelo que entrega simultáneamente `brand_id` y `model_id`; no declara coberturas BRAND y MODEL independientes. El universo y los denominadores siguen siendo explícitos. RVM no admite STORE, SELLER ni CHANNEL y no infiere territorio hacia tiendas.
...
`MANAGED` exige que `Gestionado el` sea una fecha parseable. `UNMANAGED` es su complemento exacto dentro de la cohorte: incluye campo ausente/vacío y campo presente pero no parseable. Este último caso se cuantifica por separado en `coverage.eventDateCoverage.invalidManagedDateRecords` y, cuando es mayor que cero, emite `CRM_INVALID_MANAGED_DATE_PRESENT`. `MANAGEMENT_COVERAGE = MANAGED / (MANAGED + UNMANAGED)`. `CONVERSION_ON_MANAGED = (Vendido afirmativo AND MANAGED) / MANAGED`; un SOLD sin fecha válida de gestión no entra al numerador ni al denominador managed.
