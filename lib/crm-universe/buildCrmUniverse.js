  ...
  return `WITH source AS MATERIALIZED (
    SELECT c.*, count(*) OVER()::bigint source_rows,
      row_number() OVER (
        ...
        ORDER BY ${dateExpression('"loaded_at"')} DESC NULLS LAST, ctid DESC
      ) rn
    FROM "CRM_Cidef_raw" c
  ),
  dedup AS MATERIALIZED (SELECT * FROM source WHERE rn=1),
...
export function assembleCrmUniverse(rows, input = {}) {
  ...
  const events = commercialUniverse === 'OWN_STORES'
    ? all.filter((row) => row.store_resolution_status === 'RESOLVED' && row.tipo_canal === 'CIDEF') : all;
  ...
  const validation = {
    ...
    analytical_events_count: events.length,
  ...
  return {
    ...
    analytical_events: events, coverage, validation,
    warnings: ['CRM_NO_HISTORICAL_STATE_SNAPSHOTS', ...(commercialUniverse === 'OWN_STORES' ? ['OWN_STORES_REQUIRES_RESOLVED_CIDEF_STORE'] : [])],
    lineage: {
      source: 'CRM_Cidef_raw', deduplication: 'latest loaded row per non-empty CRM ID; stable ctid tie-break',
