...
async function parseSharedStrings(entry) {
  ...
  await parseXmlEntry(entry, {
    opentag(node) {
      if (node.name === 'si') { inSi = true; text = ''; }
      else if (inSi && node.name === 't') inT = true;
    },
...
async function sheetPathByName(zip, requestedName) {
  ...
  await parseXmlEntry(workbookEntry, {
    opentag(node) {
      if (node.name === 'sheet') {
        sheets.push({ name: node.attributes.name, relId: node.attributes['r:id'] });
      }
    },
  });
  ...
  await parseXmlEntry(relsEntry, {
    opentag(node) {
      if (node.name === 'Relationship') rels.set(node.attributes.Id, node.attributes.Target);
    },
  });
...
function attachRowParser(stream, sharedStrings, onRow, onDone, onError) {
  ...
  parser.on('opentag', (node) => {
    if (node.name === 'row') currentRow = { number: Number(node.attributes.r || 0), values: [] };
    else if (node.name === 'c' && currentRow) {
      currentCell = { index: colIndex(node.attributes.r || ''), type: node.attributes.t || '', raw: '', inline: '' };
    } else if (node.name === 'v' && currentCell) captureV = true;
    else if (node.name === 't' && currentCell?.type === 'inlineStr') captureT = true;
  });
