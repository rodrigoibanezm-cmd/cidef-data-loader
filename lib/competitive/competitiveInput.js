...
const ORIGIN_GROUPS = new Set(['CHINESE', 'NON_CHINESE', 'UNKNOWN']);
...
function originGroup(value) {
  if (value == null) return null;
  const normalized = String(value).toUpperCase();
  if (!ORIGIN_GROUPS.has(normalized)) {
    throw new Error('origin_group must be CHINESE, NON_CHINESE or UNKNOWN');
  }
  return normalized;
}
...
export function parseCompetitiveInput(input = {}) {
  ...
  return {
    ...
    originGroup: originGroup(input.origin_group),
  };
}
