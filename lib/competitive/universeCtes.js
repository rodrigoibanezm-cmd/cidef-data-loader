...
universe_model_units AS (
  SELECT u.segment_key, u.type_key, u.fuel_key, u.segment, u.type, u.fuel, u.target_model_ids,
    CASE
      WHEN i.model_id = ANY(u.target_model_ids) AND i.raw_brand_norm<>'DFM'
        THEN 'RAW:'||coalesce(i.raw_brand_norm,'')||'|'||coalesce(i.raw_model_norm,'')
      WHEN i.model_id IS NOT NULL THEN 'MODEL:'||i.model_id
      ELSE 'RAW:'||coalesce(i.raw_brand_norm,'')||'|'||coalesce(i.raw_model_norm,'')
    END AS entity_key,
    CASE
