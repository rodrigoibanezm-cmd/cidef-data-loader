...
function salesTrendContext(trend, targetMonth) {
  const values = (trend || []).map(Number);
  const periods = values.map((_, index) => shiftMonth(targetMonth, index - values.length + 1));
  if (values.length < 2) return { trend_periods: periods, trend_tone: 'neutral', trend_description: 'Sin tendencia comparable' };
  const lastIndex = values.length - 1;
  const lastSign = Math.sign(values[lastIndex] - values[lastIndex - 1]);
  if (lastSign === 0) {
    return { trend_periods: periods, trend_tone: 'warning', trend_description: 'Sin cambio frente al mes anterior' };
  }
  let startIndex = lastIndex - 1;
  while (startIndex > 0 && Math.sign(values[startIndex] - values[startIndex - 1]) === lastSign) startIndex -= 1;
  const transitions = lastIndex - startIndex;
  const absoluteChange = values[lastIndex] - values[startIndex];
  const signedChange = (absoluteChange > 0 ? '+' : '') + absoluteChange + ' VIN';
  const since = monthName(periods[startIndex]);
  if (lastSign > 0) {
    const previousPeak = Math.max(...values.slice(0, startIndex));
    const verb = previousPeak > values[lastIndex] ? 'Recuperándose' : 'Subiendo';
    return {
      trend_periods: periods,
      trend_tone: transitions >= 2 ? 'positive' : 'warning',
      trend_description: verb + ' desde ' + since + ' (' + signedChange + ')',
    };
  }
  return {
    trend_periods: periods,
    trend_tone: transitions >= 2 ? 'critical' : 'warning',
    trend_description: 'Bajando desde ' + since + ' (' + signedChange + ')',
  };
}
...
export async function materializeDashboard({ requestedDate } = {}) {
  ...
  const observedSaleDates = (ventas.sales || []).map(saleDate).filter((date) => date && date <= requested).sort();
  const cutoff = observedSaleDates.at(-1) || ventas.source_context?.effective_cutoff_date || requested;
  ...
  for (const store of storesRows) {
    ...
    const salesCard = {
      ...card('VENTAS', sales.current, 'VIN', labelFor(sales.delta), toneFor(sales.delta),
        ...
        cutoff, sales.trend),
      ...salesTrendContext(sales.trend, month),
    };
  ...
  const companyCards = {
    sales: {
      ...card('VENTAS', salesCompany.current, 'VIN', labelFor(salesDelta), toneFor(salesDelta),
      ...
      ...salesTrendContext(salesCompany.trend, month),
    },
    rvm: rvm.card,
  ...
  const insertRows = await sql.query(`
    WITH company_insert AS (
      ...
      VALUES ($1::date,$2::date,'dashboard_snapshot_v01_2',$3::jsonb,true)
