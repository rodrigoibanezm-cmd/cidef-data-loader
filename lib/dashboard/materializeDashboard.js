...
async function loadProjection(sql, month, sales, cutoff) {
...
export async function materializeDashboard({ requestedDate } = {}) {
  ...
  const [crmRows, forumRows, projection, rvm] = await Promise.all([
    ...
    `, [from, requested]),
    loadProjection(sql, month, ventas.sales || [], cutoff),
  ...
  const alertCount = signals.filter((s) => s.signal_type === 'ACTION' && ['CRITICAL','WARNING'].includes(s.tone)).length;
  ...
  const insertRows = await sql.query(`
    WITH company_insert AS (
      ...
      VALUES ($1::date,$2::date,'dashboard_snapshot_v01',$3::jsonb,true)
