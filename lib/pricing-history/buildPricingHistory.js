...
function overlaps(row, dateFrom, dateTo) {
  const from = isoDate(row.vigencia_desde);
  const to = isoDate(row.vigencia_hasta);
...
export function buildPricingHistory({ identity, episodeRows = [], vinRows = [] }, parsed) {
  ...
  const seen = new Set();
  const vinByEpisode = new Map(vinRows.map((row) => [String(row.price_episode_id), row]));
  ...
  for (const row of scoped) {
    ...
    const vin = vinByEpisode.get(key);
    const canonicalCount = integerOrNull(row.vin_facturados_en_vigencia) ?? 0;
    const actualCount = vin ? integerOrNull(vin.vin_count) ?? 0 : 0;
    if (canonicalCount !== actualCount || !sameNullableDate(row.primer_vin_fecha, vin?.primer_vin_fecha) || !sameNullableDate(row.ultimo_vin_fecha, vin?.ultimo_vin_fecha)) {
      throw new Error(`VIN_RECONCILIATION_FAILED: ${key}`);
    }
    if (row.source_status === 'SOURCE_CONFLICT' && actualCount !== 0) throw new Error(`SOURCE_CONFLICT_HAS_VIN: ${key}`);
  }
  ...
  const coverage = {
    ...
    vin_before_first_episode: null,
    vin_in_conflict_window: null,
    coverage_ratio: null,
    status: 'PARTIAL',
  ...
  const visible = parsed.includeConflicts ? scoped : scoped.filter((row) => row.source_status !== 'SOURCE_CONFLICT');
  const history = visible.map((row) => ({
    price_episode_id: integerOrNull(row.price_episode_id),
    version_id: versionId,
    brand: identity.brand,
    model: identity.model,
    version: identity.version,
    vigencia_desde: isoDate(row.vigencia_desde),
    vigencia_hasta: isoDate(row.vigencia_hasta),
    days_active: daysInclusive(isoDate(row.vigencia_desde), isoDate(row.vigencia_hasta)),
    precio_neto: integerOrNull(row.precio_neto),
    precio_lista: integerOrNull(row.precio_lista),
    precio_con_iva: integerOrNull(row.precio_con_iva),
    bono_cidef: integerOrNull(row.bono_cidef),
    bono_forum: integerOrNull(row.bono_forum),
    bono_mes: integerOrNull(row.bono_mes),
    delta_precio_neto: integerOrNull(row.delta_precio_neto),
    delta_precio_lista: integerOrNull(row.delta_precio_lista),
    delta_precio_con_iva: integerOrNull(row.delta_precio_con_iva),
    delta_bono_cidef: integerOrNull(row.delta_bono_cidef),
    delta_bono_forum: integerOrNull(row.delta_bono_forum),
    delta_bono_mes: integerOrNull(row.delta_bono_mes),
    tipo_cambio: row.tipo_cambio,
    vin_facturados: integerOrNull(row.vin_facturados_en_vigencia) ?? 0,
    primer_vin_fecha: isoDate(row.primer_vin_fecha),
    ultimo_vin_fecha: isoDate(row.ultimo_vin_fecha),
  ...
  return {
    ...
    summary: {
      ...
      vin_facturados: history.reduce((sum, row) => sum + row.vin_facturados, 0),
    },
    history,
  };
}
