...
export function validateInverseShareMovements({
  ...
  const validation = {
    ...
    evidence_filter_reconciles: projection.page.totalRelations === rows.length
      && projection.page.eligibleRelations + projection.page.excludedByMinEvidence === rows.length
      && projection.page.eligibleRelations === rows.filter((row) => (
        parsed.minEvaluableTransitions == null
        || row.jointEvaluableTransitions >= parsed.minEvaluableTransitions
      )).length,
    pagination_after_evidence_filter: projection.page.returnedRelations === projection.movements.length
      && projection.movements.every((row) => parsed.minEvaluableTransitions == null
        || row.jointEvaluableTransitions >= parsed.minEvaluableTransitions),
