...
seller_best AS (
  ...
  FROM seller_max
  WHERE token_score = max_score AND max_score >= 2
),
seller_unique AS (
...
model_ranked AS (
  SELECT mc.*, dense_rank() OVER (PARTITION BY crm_opportunity_id ORDER BY specificity DESC) AS rnk
...
export async function findCompatibleOpportunities(sql, { sucursalId, personaId, modeloId, crmOpportunityId = null }) {
  return sql.query(`${resolutionCtes}
    SELECT
