...
export function assembleOrganizationalContext(ventasContext, series, scope) {
  ...
  const storeMismatch = events.filter((row) =>
    row.seller_eligibility_status === 'VENDEDOR_CIDEF_STORE_MISMATCH').length;
  ...
  const validation = {
    ...
    seller_categories_reconcile: eligible + nonEligible + storeMismatch
    ...
    seller_temporal_membership_verified: series.sellerMonthly.every((row) =>
      row.temporal_membership_verified === true),
    seller_observed_store_assignment_matches: series.sellerMonthly.every((row) =>
  ...
  const cutoffMonth = ventasContext.cutoff_month ?? null;
  const recognizedAfterCutoff = cutoffMonth == null ? 0 : (ventasContext.recognizedSales || [])
    .filter((row) => row.mes_venta > cutoffMonth).length;
  ...
  return {
    ...
    seller_attribution_monthly: series.sellerAttributionMonthly,
    store_identity_monthly: series.storeIdentityMonthly,
    ...
    coverage: {
      ...
      seller_store_mismatch_sales: storeMismatch,
    ...
    temporal_evidence: {
      cutoff_month: cutoffMonth,
      rows_inside_cutoff: ventasContext.coverage?.rows_inside_cutoff ?? null,
      rows_excluded_by_cutoff: ventasContext.coverage?.rows_excluded_by_cutoff ?? null,
      recognized_sales_after_cutoff: recognizedAfterCutoff,
    },
    validation,
    warnings,
  };
