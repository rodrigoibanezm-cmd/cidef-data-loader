...
export function enrichRecognizedSale(sale, identityMaps) {
  ...
  const dealer = storeStatus === 'RESUELTA' && store.dealer_id != null
    ? identityMaps?.dealers?.get(String(store.dealer_id)) : null;
  ...
  return {
    ...
    dealer_id: storeStatus === 'RESUELTA' ? store.dealer_id ?? null : null,
    dealer_nombre: dealer?.nombre_canonico ?? null,
    dealer_group_id: storeStatus === 'RESUELTA' ? store.dealer_group_id ?? null : null,
    dealer_group_nombre: dealer?.dealer_group_nombre ?? null,
