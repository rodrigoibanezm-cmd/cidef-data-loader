...
export async function resolveCanonicalEntity(entity, options = {}) {
  ...
  if (type === 'BRAND') {
    ...
    rows = await sql.query(`SELECT DISTINCT v.version_id AS canonical_id, v.nombre_canonico AS display_name FROM public.versiones_master_v01 v LEFT JOIN public.producto_aliases_v01 a ON a.version_id=v.version_id AND a.nivel='VERSION' AND a.estado='RESUELTO' WHERE upper(v.nombre_canonico)=upper($1) OR upper(v.nombre_normalizado)=upper($2) OR upper(a.valor_raw)=upper($1) OR upper(a.valor_normalizado)=upper($2) ORDER BY v.version_id LIMIT 10`, [value, key]);
  } else if (type === 'STORE') {
