...
const AAD = Buffer.from('cidef-resolution.v1');
export function issueResolutionToken(authority, options={}) {
  const now = options.nowMs ?? Date.now();
  const payload = Buffer.from(JSON.stringify({ v:1, iat:now, exp:now+(options.ttlMs ?? 30*60*1000), authority }));
...
export function verifyResolutionToken(token, options={}) {
  ...
  try {
    ...
    decipher.setAAD(AAD); decipher.setAuthTag(dec(tagRaw));
    payload=JSON.parse(Buffer.concat([decipher.update(dec(cipherRaw)),decipher.final()]).toString('utf8'));
  } catch {
    throw Object.assign(new Error('INVALID_RESOLUTION_ID_SIGNATURE'),{code:'INVALID_RESOLUTION_ID_SIGNATURE'});
  }
  const now=options.nowMs??Date.now();
  if(payload.v!==1 || !payload.exp || payload.exp<now) throw Object.assign(new Error('RESOLUTION_ID_EXPIRED'),{code:'RESOLUTION_ID_EXPIRED'});
