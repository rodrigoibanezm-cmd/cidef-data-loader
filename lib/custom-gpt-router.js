...
function validateLongitudinalRequest(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) {
    throw routingError('INVALID_LONGITUDINAL_CONTEXT');
  }
  const domain = String(value.domain || '').trim().toUpperCase();
  if (!LONGITUDINAL_ACTION_BY_DOMAIN[domain]) {
    throw routingError('INVALID_LONGITUDINAL_DOMAIN', String(value.domain ?? 'missing'));
  }
  const allowedFields = new Set([
    ...
    'cutoff_date', 'cutoff_mode', 'commercial_universe',
  ]);
  ...
  for (const field of ['metric', 'grain', 'filters', 'date_from', 'date_to', 'time_grain']) {
    if (value[field] == null) throw routingError('MISSING_LONGITUDINAL_CONTEXT_FIELD', field);
  }
  if (!value.filters || typeof value.filters !== 'object' || Array.isArray(value.filters)) {
    throw routingError('INVALID_LONGITUDINAL_CONTEXT_FIELD', 'filters');
  }
  if (domain === 'CRM') {
    ...
    if (mode === 'EVENT' && value.date_axis == null) {
      throw routingError('MISSING_LONGITUDINAL_CONTEXT_FIELD', 'date_axis');
    }
    if (mode === 'COHORT' && value.cohort_axis == null) {
      throw routingError('MISSING_LONGITUDINAL_CONTEXT_FIELD', 'cohort_axis');
    }
  }
...
function validateCrossDomainCommercialCompatibility(analyticalResult, longitudinalContext) {
  const outputs = [analyticalResult, longitudinalContext];
  const ventas = outputs.find((output) => output?.domain === 'VENTAS');
  const crm = outputs.find((output) => output?.domain === 'CRM');
  if (!ventas || !crm) return;

  const ventasCommercialUniverse = ventas.commercial_scope?.universe;
  const crmCommercialUniverse = crm.commercial_scope?.universe;
  if (ventasCommercialUniverse == null || crmCommercialUniverse == null) return;
...
export async function runCustomGptCapability(request = {}, executor = runCustomGptAction) {
  const resolved = resolveDomainCapability(request.domain, request.capability);
  const input = request.input ?? {};
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw routingError('INVALID_CAPABILITY_INPUT');
  }
...
export async function runCustomGptActionWithContext(request = {}, executor = runCustomGptAction) {
  ...
  if (flag != null && typeof flag !== 'boolean') {
    throw routingError('INVALID_REQUIRES_LONGITUDINAL_CONTEXT');
  }
  ...
  const [analyticalResult, longitudinalContext] = await Promise.all([
    executor(request.action, request.input ?? {}),
    executor(longitudinal.action, longitudinal.input),
  ]);
