...
function fingerprintFor(record) {
  return {
    ...
    cc: firstText(record, ['cc', 'cilindrada']),
    hp: firstText(record, ['hp', 'potencia_hp']),
    combustible: firstText(record, ['combustible']),
    traccion: firstText(record, ['traccion']),
    carga_kg: firstText(record, ['carga_kg', 'carga_kg_2', 'carga']),
    pasajeros: firstText(record, ['pasajeros', 'pasajero']),
    euro: firstText(record, ['euro', 'norma_euro']),
  };
}
...
export function normalizePriceRecord(record) {
  ...
  // A canonical commercial version must come from a priced row.
