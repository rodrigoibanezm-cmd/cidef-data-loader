...
export function buildGenerationsQuery(scope) {
  const filters = [];
  const params = [];
  addFilter(filters, params, 'g.modelo_id', scope.modeloId);
  ...
  return {
    sql: `
      ...
      JOIN modelos_master_v01 m ON m.modelo_id=g.modelo_id
      LEFT JOIN version_generation_v01 vg ON vg.generation_id=g.generation_id
      ${filters.length ? `WHERE ${filters.join(' AND ')}` : ''}
      GROUP BY g.generation_id, g.modelo_id, m.nombre_canonico,
               g.nombre_canonico, g.nombre_normalizado
      ORDER BY g.modelo_id, g.generation_id
      LIMIT $${params.length}
    `,
    params,
  };
