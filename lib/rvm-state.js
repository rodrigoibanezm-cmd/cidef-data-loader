...
export async function appendRvmRows(columns, rows, fileName, lastRow) {
  ...
  await db.query(`INSERT INTO ${q(STAGING)} (${columnList}) VALUES ${tuples.join(', ')}`, values);
  await db.query(`UPDATE ${q(PROGRESS)} SET last_row = $2, rows_loaded = rows_loaded + $3, updated_at = NOW() WHERE file_name = $1`, [fileName, lastRow, rows.length]);
}
