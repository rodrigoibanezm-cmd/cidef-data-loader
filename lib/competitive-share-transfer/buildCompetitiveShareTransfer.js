...
export async function buildCompetitiveShareTransfer(input = {}, dependencies = {}) {
  ...
  if (parsed.mode === 'HISTORICAL') {
    ...
    if (inventory.length > 1) return unevaluable(parsed, 'MULTIPLE_PRELIMINARY_SNAPSHOTS', inventory);
    const selected = inventory[0];
    if (!selected || selected.snapshot_date !== parsed.dateCutoff
      || selected.max_date !== parsed.dateCutoff || selected.min_date < ranges.current.dateFrom) {
      return unevaluable(parsed, 'NO_VALID_PRELIMINARY_SNAPSHOT', inventory);
    }
