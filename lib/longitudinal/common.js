...
export function buildTemporalSemantics(parsed, lastObservedDate = null) {
  const observed = lastObservedDate && validDate(String(lastObservedDate).slice(0, 10))
    ? String(lastObservedDate).slice(0, 10) : null;
...
export function identityWarnings(dimensionCoverage = []) {
  ...
  for (const row of dimensionCoverage) {
    if (row.unresolved > 0) warnings.push(`${row.dimension}_IDENTITY_UNRESOLVED_PRESENT`);
    if (row.ambiguous > 0) warnings.push(`${row.dimension}_IDENTITY_AMBIGUOUS_PRESENT`);
  }
  return warnings;
}
