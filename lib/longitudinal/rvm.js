...
import { buildRvmUniverseCtes, RVM_UNIVERSE_FILTERS } from '../rvm-universe/buildRvmUniverse.js';
...
const DIMS = Object.freeze({
  BRAND: { key: `CASE WHEN r.scope_brand_id IS NOT NULL THEN r.scope_brand_id::text WHEN r.identity_status='AMBIGUO' THEN 'AMBIGUOUS' ELSE 'UNRESOLVED' END`, label: `CASE WHEN r.scope_brand_id IS NOT NULL THEN r.brand_name WHEN r.identity_status='AMBIGUO' THEN 'AMBIGUOUS' ELSE 'UNRESOLVED' END` },
  MODEL: { key: `CASE r.identity_status WHEN 'RESUELTO' THEN r.model_id::text WHEN 'AMBIGUO' THEN 'AMBIGUOUS' ELSE 'UNRESOLVED' END`, label: `CASE r.identity_status WHEN 'RESUELTO' THEN r.model_name WHEN 'AMBIGUO' THEN 'AMBIGUOUS' ELSE 'UNRESOLVED' END` },
...
function entityPredicates(filters, params, alias = 'r') {
  const columns = {
...
export function buildRvmLongitudinalQuery(parsed) {
  const params = [];
  const universe = buildRvmUniverseCtes(parsed, params);
  ...
  const entity = entityPredicates(parsed.entity, params);
  ...
  const groups = bd
    ? `SELECT period,'TOTAL'::text row_type,NULL::text bucket_key,NULL::text bucket_label,sum(cantidad)::numeric units FROM base GROUP BY period
       UNION ALL SELECT period,'BREAKDOWN',${bd.key},min(${bd.label}),sum(cantidad)::numeric FROM base r GROUP BY period,${bd.key}`
    : `SELECT period,'TOTAL'::text row_type,NULL::text bucket_key,NULL::text bucket_label,sum(cantidad)::numeric units FROM base GROUP BY period`;
  const entityGroups = bd
    ? `SELECT period,'TOTAL'::text row_type,NULL::text bucket_key,sum(cantidad)::numeric units FROM entity_base GROUP BY period
       UNION ALL SELECT period,'BREAKDOWN',${bd.key},sum(cantidad)::numeric FROM entity_base r GROUP BY period,${bd.key}`
    : `SELECT period,'TOTAL'::text row_type,NULL::text bucket_key,sum(cantidad)::numeric units FROM entity_base GROUP BY period`;
  let rankCtes = '';
  ...
  const sql = `WITH ${universe.ctes},
  calendar AS (SELECT to_char(d,'${format}') period FROM generate_series(date_trunc('${trunc}',${dateFrom}::date),date_trunc('${trunc}',${dateTo}::date),'${interval}'::interval) d),
  universe_pre_org AS MATERIALIZED (SELECT i.*,to_char(date_trunc('${trunc}',i.fecha),'${format}') period FROM ${universe.relation} i),
  ...
  target_base AS MATERIALIZED (SELECT r.* FROM base r${entity.length ? ` WHERE ${entity.join(' AND ')}` : ''}),
