...
const RAW = Object.freeze({
  product_interest: '"Producto de interes"', origin: '"Origen"',
...
function filterSql(filters, params) {
  const columns = {
    ...RAW, brand: 'e.brand', store_id: 'e.sucursal_id::text', store: 'e.sucursal_nombre',
...
function identityDimension(grain) {
  if (grain === 'BRAND') return {
    key: `CASE WHEN product_interest_norm IS NULL THEN 'NOT_APPLICABLE' WHEN brand_match_count>1 THEN 'AMBIGUOUS' WHEN brand IS NULL THEN 'UNRESOLVED' ELSE brand END`,
    label: `CASE WHEN product_interest_norm IS NULL THEN 'NOT_APPLICABLE' WHEN brand_match_count>1 THEN 'AMBIGUOUS' WHEN brand IS NULL THEN 'UNRESOLVED' ELSE brand END`,
    status: `CASE WHEN product_interest_norm IS NULL THEN 'NOT_APPLICABLE' WHEN brand_match_count>1 THEN 'AMBIGUOUS' WHEN brand IS NULL THEN 'UNRESOLVED' ELSE 'RESOLVED' END`, raw: 'product_interest_raw',
  };
...
export function buildCrmLongitudinalQuery(parsed) {
  ...
  const domainPredicate = parsed.commercialUniverse === 'OWN_STORES'
    ? "store_match_count=1 AND tipo_canal='CIDEF'" : 'TRUE';
  ...
  product_candidates AS MATERIALIZED (
    SELECT master_norm(m.nombre_canonico) norm,m.modelo_id,ma.marca_id,ma.nombre_canonico brand FROM modelos_master_v01 m JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT master_norm(concat_ws(' ',ma.nombre_canonico,m.nombre_canonico)),m.modelo_id,ma.marca_id,ma.nombre_canonico FROM modelos_master_v01 m JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT master_norm(v.nombre_canonico),m.modelo_id,ma.marca_id,ma.nombre_canonico FROM versiones_master_v01 v JOIN modelos_master_v01 m ON m.modelo_id=v.modelo_id JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT master_norm(concat_ws(' ',ma.nombre_canonico,v.nombre_canonico)),m.modelo_id,ma.marca_id,ma.nombre_canonico FROM versiones_master_v01 v JOIN modelos_master_v01 m ON m.modelo_id=v.modelo_id JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT master_norm(concat_ws(' ',m.nombre_canonico,v.nombre_canonico)),m.modelo_id,ma.marca_id,ma.nombre_canonico FROM versiones_master_v01 v JOIN modelos_master_v01 m ON m.modelo_id=v.modelo_id JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT master_norm(concat_ws(' ',ma.nombre_canonico,m.nombre_canonico,v.nombre_canonico)),m.modelo_id,ma.marca_id,ma.nombre_canonico FROM versiones_master_v01 v JOIN modelos_master_v01 m ON m.modelo_id=v.modelo_id JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id
    UNION SELECT a.valor_normalizado,coalesce(a.modelo_id,v.modelo_id),ma.marca_id,ma.nombre_canonico FROM producto_aliases_v01 a LEFT JOIN versiones_master_v01 v ON v.version_id=a.version_id JOIN modelos_master_v01 m ON m.modelo_id=coalesce(a.modelo_id,v.modelo_id) JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id WHERE a.estado='RESUELTO' AND a.nivel IN ('MODELO','VERSION')
    UNION SELECT master_norm(concat_ws(' ',ma.nombre_canonico,a.valor_raw)),coalesce(a.modelo_id,v.modelo_id),ma.marca_id,ma.nombre_canonico FROM producto_aliases_v01 a LEFT JOIN versiones_master_v01 v ON v.version_id=a.version_id JOIN modelos_master_v01 m ON m.modelo_id=coalesce(a.modelo_id,v.modelo_id) JOIN marcas_master_v01 ma ON ma.marca_id=m.marca_id WHERE a.estado='RESUELTO' AND a.nivel IN ('MODELO','VERSION')
  ),
  product_map AS MATERIALIZED (SELECT norm,min(modelo_id) modelo_id,min(marca_id) marca_id,min(brand) brand,count(DISTINCT modelo_id)::int model_match_count,count(DISTINCT marca_id)::int brand_match_count FROM product_candidates WHERE norm IS NOT NULL GROUP BY norm),
  ...
  dated AS MATERIALIZED (SELECT d.*,${dateExpression(axis)} event_date,${dateExpression('"Gestionado el"')} managed_date,${dateExpression('"Desistido el"')} desist_date,master_norm("Producto de interes") product_interest_norm,"Producto de interes" product_interest_raw,master_norm("Sucursal Asignada") store_raw_norm,master_norm("Asignado a") seller_raw_norm,"Sucursal Asignada" store_raw,"Asignado a" seller_raw FROM dedup d),
  enriched AS MATERIALIZED (SELECT d.*,pr.model_match_count,pr.brand_match_count,CASE WHEN pr.model_match_count=1 THEN pr.modelo_id END product_model_id,CASE WHEN pr.brand_match_count=1 THEN pr.marca_id END brand_id,CASE WHEN pr.brand_match_count=1 THEN pr.brand END brand,sm.match_count store_match_count,CASE WHEN sm.match_count=1 THEN sm.sucursal_id END sucursal_id,ss.nombre_canonico sucursal_nombre,ss.tipo_canal,pm.match_count seller_match_count,CASE WHEN pm.match_count=1 THEN pm.persona_id END persona_id,pp.nombre_canonico persona_nombre,
    ...
    FROM dated d LEFT JOIN product_map pr ON pr.norm=d.product_interest_norm LEFT JOIN store_map sm ON sm.norm=d.store_raw_norm LEFT JOIN sucursales_master ss ON ss.sucursal_id=sm.sucursal_id AND sm.match_count=1 LEFT JOIN seller_map pm ON pm.norm=d.seller_raw_norm LEFT JOIN personas_master pp ON pp.persona_id=pm.persona_id AND pm.match_count=1),
  domain_scoped AS MATERIALIZED (SELECT e.* FROM enriched e WHERE ${domainPredicate}),
  filtered AS MATERIALIZED (SELECT e.* FROM domain_scoped e WHERE ${filters.length ? filters.join(' AND ') : 'TRUE'}),
  ...
  grouped AS (${groups}),
  commercial_coverage AS (SELECT count(*)::bigint crm_total_records,count(*) FILTER(WHERE ${domainPredicate})::bigint included_in_domain,count(*) FILTER(WHERE store_match_count=1 AND tipo_canal IS DISTINCT FROM 'CIDEF')::bigint excluded_other_universe,count(*) FILTER(WHERE store_raw_norm IS NOT NULL AND store_match_count IS NULL)::bigint commercial_identity_unresolved,count(*) FILTER(WHERE store_match_count>1)::bigint commercial_identity_ambiguous,count(*) FILTER(WHERE store_raw_norm IS NULL)::bigint commercial_identity_not_applicable FROM enriched),
  ...
  identity_coverage AS (SELECT count(*) FILTER(WHERE product_interest_norm IS NOT NULL AND brand_match_count=1)::bigint brand_resolved,count(*) FILTER(WHERE product_interest_norm IS NOT NULL AND brand_match_count IS NULL)::bigint brand_unresolved,count(*) FILTER(WHERE brand_match_count>1)::bigint brand_ambiguous,count(*) FILTER(WHERE product_interest_norm IS NULL)::bigint brand_not_applicable,count(*) FILTER(WHERE store_raw_norm IS NOT NULL AND store_match_count=1)::bigint store_resolved,count(*) FILTER(WHERE store_raw_norm IS NOT NULL AND store_match_count IS NULL)::bigint store_unresolved,count(*) FILTER(WHERE store_match_count>1)::bigint store_ambiguous,count(*) FILTER(WHERE store_raw_norm IS NULL)::bigint store_not_applicable,count(*) FILTER(WHERE seller_raw_norm IS NOT NULL AND seller_match_count=1 AND eligible_vendedor_cidef)::bigint seller_resolved,count(*) FILTER(WHERE seller_raw_norm IS NOT NULL AND seller_match_count IS NULL)::bigint seller_unresolved,count(*) FILTER(WHERE seller_match_count>1)::bigint seller_ambiguous,count(*) FILTER(WHERE seller_raw_norm IS NULL OR (seller_match_count=1 AND NOT eligible_vendedor_cidef))::bigint seller_not_applicable,count(*)::bigint identity_total FROM scoped)
  SELECT c.period,x.row_type,x.bucket_key,x.bucket_label,x.identity_status,x.raw_values,coalesce(g.numerator,0)::numeric numerator,coalesce(g.denominator,0)::numeric denominator,${value} value,a.last_observed_date,a.effective_date_to,a.comparison_day,v.*,i.*,cc.*
  ...
  LEFT JOIN grouped g ON g.period=c.period AND g.row_type=x.row_type AND g.bucket_key IS NOT DISTINCT FROM x.bucket_key CROSS JOIN anchor a CROSS JOIN axis_coverage v CROSS JOIN identity_coverage i CROSS JOIN commercial_coverage cc
...
export function assembleCrmLongitudinal(parsed, rows) {
  ...
  const dimensionCoverage = ['BRAND', 'STORE', 'SELLER'].map((dimension) => coverageRow(dimension, {
  ...
  const commercialCoverage = {
    totalCrmRecordsConsidered: Number(first.crm_total_records || 0), includedInDomain: Number(first.included_in_domain || 0),
    excludedOtherUniverse: Number(first.excluded_other_universe || 0), unresolved: Number(first.commercial_identity_unresolved || 0),
    ambiguous: Number(first.commercial_identity_ambiguous || 0), notApplicable: Number(first.commercial_identity_not_applicable || 0),
  };
  ...
  const result = outputEnvelope({ motor: ENGINE_NAME, domain: 'CRM', parsed, series, seriesByBreakdown, temporalSemantics, coverage: { dimensionCoverage, eventDateCoverage }, warnings, metadata: {
  ...
  return {
    ...result,
    commercial_scope: { universe: parsed.commercialUniverse, authority: 'Sucursal Asignada exact MASTER identity via sucursales_master.tipo_canal', valid: true, scope_id: 'crm_commercial_scope_v01' },
