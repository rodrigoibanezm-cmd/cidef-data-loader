...
export function buildCrmLongitudinalQuery(parsed) {
  ...
  product_map AS MATERIALIZED (SELECT norm,min(modelo_id) modelo_id,min(marca_id) marca_id,min(brand) brand,count(DISTINCT modelo_id)::int match_count FROM product_candidates WHERE norm IS NOT NULL GROUP BY norm),
  ...
  enriched AS MATERIALIZED (SELECT d.*,pr.match_count brand_match_count,CASE WHEN pr.match_count=1 THEN pr.modelo_id END product_model_id,CASE WHEN pr.match_count=1 THEN pr.marca_id END brand_id,CASE WHEN pr.match_count=1 THEN pr.brand END brand,sm.match_count store_match_count,CASE WHEN sm.match_count=1 THEN sm.sucursal_id END sucursal_id,ss.nombre_canonico sucursal_nombre,ss.tipo_canal,pm.match_count seller_match_count,CASE WHEN pm.match_count=1 THEN pm.persona_id END persona_id,pp.nombre_canonico persona_nombre,
