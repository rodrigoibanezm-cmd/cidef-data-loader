...
function dimension(event, grain) {
  const map = {
    ...
    SELLER: [event.eligible_vendedor_cidef ? event.persona_id : null, event.eligible_vendedor_cidef ? event.persona_nombre : null],
...
export function calculateVentasLongitudinal(events, parsed) {
  ...
  const unresolved = Object.fromEntries([...BREAKDOWNS].map((grain) => [grain, events.filter((event) => dimension(event, grain).key === 'UNRESOLVED').length]));
