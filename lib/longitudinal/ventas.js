...
import { buildVentasCommercialContext, parseCommercialUniverse } from '../ventas-commercial/buildVentasCommercialContext.js';
...
function assertDomainCompatibility(commercialUniverse, grain, breakdown, metric) {
  ...
  if (metric === 'CHANNEL_MIX_WITHIN_CIDEF') {
    ...
    if (grain !== 'TOTAL' || breakdown != null) {
      throw semanticError('SEMANTICALLY_IMPOSSIBLE_COMBINATION', 'CHANNEL_MIX_WITHIN_CIDEF requires grain=TOTAL and no breakdown; use filters to restrict brand/model/version');
    }
  }
}
...
export function parseVentasLongitudinalInput(input = {}) {
  ...
  assertDomainCompatibility(commercialUniverse, grain, breakdown, metric);
  ...
  return outputEnvelope({ motor: ENGINE_NAME, domain: 'VENTAS', parsed, series, seriesByBreakdown,
    ...
    shareDefinition: isShareWithinUniverseMetric(parsed.metric)
      ...
      : parsed.metric === 'CHANNEL_MIX_WITHIN_CIDEF'
        ? 'selected channel universe numerator / COMPANY denominator after the exact same filters in the same period'
        : null,
  } });
}
...
function enrichCommercialEvents(commercialContext, resources) {
  const recognizedSales = commercialContext.sales;
  const exact = resolveSalesModels(recognizedSales, buildProductModelResolutionMap(resources.evidence, resources.aliases));
  const aliasResolved = resolveSalesProducts(recognizedSales, resources.aliases);
  const organization = enrichRecognizedSales(recognizedSales, resources.orgMaps);
  return exact.map((sale, index) => {
    const alias = aliasResolved[index];
    const org = organization[index];
    const model = sale.modelo_id == null ? null : resources.catalogs.models.get(String(sale.modelo_id));
    const versionId = alias?.modelo_id != null && Number(alias.modelo_id) === Number(sale.modelo_id) ? alias.version_id : null;
    const version = versionId == null ? null : resources.catalogs.versions.get(String(versionId));
...
export async function buildVentasLongitudinal(input = {}) {
  ...
  const [commercialContext, companyContext, orgMaps, aliases, evidence, catalogs] = await Promise.all([
    buildVentasCommercialContext({ commercial_universe: parsed.commercialUniverse, cutoff_date: parsed.dateTo }),
    parsed.metric === 'CHANNEL_MIX_WITHIN_CIDEF'
      ? buildVentasCommercialContext({ commercial_universe: 'COMPANY', cutoff_date: parsed.dateTo })
      : Promise.resolve(null),
    loadOrganizationalIdentityMaps(sql),
    loadProductIdentityMap(sql), loadSkuEvidence(sql), loadCatalogs(sql),
  ]);
  const resources = { orgMaps, aliases, evidence, catalogs };
  const events = enrichCommercialEvents(commercialContext, resources);
  const denominatorEvents = companyContext == null ? events : enrichCommercialEvents(companyContext, resources);
  ...
  return {
    ...
    commercial_coverage: commercialContext.coverage,
    commercial_validation: commercialContext.validation,
    denominator_commercial_scope: companyContext?.commercial_scope ?? commercialContext.commercial_scope,
    denominator_commercial_coverage: companyContext?.coverage ?? commercialContext.coverage,
  };
}