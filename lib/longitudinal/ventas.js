...
function assertDomainCompatibility(commercialUniverse, grain, breakdown) {
...
export function parseVentasLongitudinalInput(input = {}) {
  ...
  assertDomainCompatibility(commercialUniverse, grain, breakdown);
...
export function calculateVentasLongitudinal(events, parsed) {
  ...
  return outputEnvelope({ motor: ENGINE_NAME, domain: 'VENTAS', parsed, series, seriesByBreakdown,
    ...
    shareDefinition: parsed.metric === 'SHARE_WITHIN_CIDEF' ? 'grain numerator / certified commercial-domain sales after all non-grain filters in the same period' : null,
  } });
}
...
export async function buildVentasLongitudinal(input = {}) {
  ...
  const [commercialContext, orgMaps, aliases, evidence, catalogs] = await Promise.all([
  ...
  const exact = resolveSalesModels(recognizedSales, buildProductModelResolutionMap(evidence, aliases));
  const aliasResolved = resolveSalesProducts(recognizedSales, aliases);
  const organization = enrichRecognizedSales(recognizedSales, orgMaps);
