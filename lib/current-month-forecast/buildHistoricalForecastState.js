...
import { learnCurrentCompletion } from '../daily-close-forecast/learnCurrentCompletion.js';
...
export function buildHistoricalForecastState(rows, identityMaps, targetMonth, dayOfMonth, now) {
  ...
  const predictability = derivePredictabilityDays(
    aggregateForecastErrors([...company, ...stores]),
  ...
  return {
    ...
    predictability,
    companyLearned: learnCurrentCompletion(context.company_observations, dayOfMonth),
    storeLearned: learnCurrentCompletion(context.store_observations, dayOfMonth),
  };
}
