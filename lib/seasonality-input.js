...
export function seasonalityParams(input) {
  const common = [input.date_from && `${input.date_from}-01`,
    input.date_to && `${shiftMonth(input.date_to, 1)}-01`, input.brand, input.model];
  const pagination = [input.page_size, (input.page - 1) * input.page_size];
  return input.scope === 'CIDEF'
    ? [...common, input.branch, input.seller, ...pagination]
    : [...common, ...pagination];
}
