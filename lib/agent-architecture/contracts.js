export const CONTRACT_VERSIONS = Object.freeze({
  ...
  analysis_iteration: 'analysis_iteration.v1',
});
...
function isoDate(value, field) {
  if (typeof value !== 'string' || !/^\d{4}-\d{2}-\d{2}$/.test(value)) fail('INVALID_SEMANTIC_PERIOD', `${field} must be YYYY-MM-DD`);
  const date = new Date(`${value}T00:00:00.000Z`);
  if (!Number.isFinite(date.getTime()) || date.toISOString().slice(0,10) !== value) fail('INVALID_SEMANTIC_PERIOD', `${field} is not a valid calendar date`);
  return value;
}
...
export function validateSemanticParse(value) {
  ...
  assertClosedObject(value.period, ['date_from','date_to'], 'INVALID_SEMANTIC_PERIOD');
  const date_from = isoDate(value.period.date_from, 'date_from');
  const date_to = isoDate(value.period.date_to, 'date_to');
  if (date_from > date_to) fail('INVALID_SEMANTIC_PERIOD', 'date_from must be <= date_to');
