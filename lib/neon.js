...
export async function replaceTableSnapshot(tableName, columns, rows) {
  ...
  const batchSize = Math.max(1, Math.min(200, Math.floor(maxParams / columns.length)));
  ...
  return { rowsLoaded: rows.length, columnsLoaded: columns.length, table: tableName, batchSize };
}
