...
export function buildStatePoints(historyRows, resolutionMap) {
  ...
  for (const rows of groups.values()) {
    const first = rows[0];
    const priceVersionId = Number(first.price_version_id);
    const tuples = new Map();
    for (const row of rows) {
      const tuple = commercialTuple(row);
      tuples.set(tupleKey(tuple), tuple);
    }
    const variantCount = tuples.size;
    const clean = variantCount === 1;
    const resolution = resolutionMap.get(priceVersionId);
    points.push({
      price_version_id: priceVersionId,
      version_id: resolution?.version_id ?? null,
      resolution_status: resolution?.status ?? 'unresolved',
      vigencia_desde: dateOnly(first.vigencia_desde),
      source_status: clean ? 'OK' : 'SOURCE_CONFLICT',
      tuple: clean ? [...tuples.values()][0] : null,
      source_rows: rows.length,
      source_variant_count: variantCount,
      source_files: [...new Set(rows.map((row) => row.source_file).filter(Boolean))].sort(),
    });
  }
  points.sort((a, b) => a.price_version_id - b.price_version_id || a.vigencia_desde.localeCompare(b.vigencia_desde));
  return points;
}
...
export function buildPriceEpisodes(statePoints) {
  ...
  for (const point of statePoints) {
    ...
    if (!byVersion.has(point.price_version_id)) byVersion.set(point.price_version_id, []);
    byVersion.get(point.price_version_id).push(point);
  }

  const episodes = [];
  for (const [, points] of byVersion) {
    points.sort((a, b) => a.vigencia_desde.localeCompare(b.vigencia_desde));
    const segments = [];
    let current = null;
  ...
  return episodes.sort((a, b) => a.price_version_id - b.price_version_id || a.vigencia_desde.localeCompare(b.vigencia_desde));
}
...
export function assignVehiclesToEpisodes(vehicleRows, episodes) {
  ...
  for (const vehicle of vehicleRows) {
    ...
    if (matches.length > 1) {
      ambiguous.push({ vehiculo_id: Number(vehicle.vehiculo_id), vin: vehicle.vin, matches: matches.map((e) => `${e.price_version_id}|${e.vigencia_desde}`) });
      continue;
    }
    const episode = matches[0];
    assignments.push({
      episode_key: `${episode.price_version_id}|${episode.vigencia_desde}`,
...
export function addEpisodeVinAggregates(episodes, assignments) {
  ...
  return episodes.map((episode) => {
    const key = `${episode.price_version_id}|${episode.vigencia_desde}`;
    const item = aggregate.get(key) ?? { count: 0, min: null, max: null };
    return {
      ...episode,
...
BEGIN
  ...
  JOIN public.price_episode_canonico_v01 b
    ON a.price_version_id=b.price_version_id
...
async function postAudit(expectedConflicts) {
  const rows = await queryDb(`
    ...
    SELECT
      ...
      (SELECT n FROM overlaps) AS overlaps,
