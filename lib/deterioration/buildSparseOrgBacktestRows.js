...
import { firstObservedMonth } from './orgSalesSeries.js';
...
export function buildSparseOrgBacktestRows(snapshotSet, parsed) {
  const allRows = [];
  const rows = [];
  const priorErrors = new Map();
  const skipped = Object.fromEntries(parsed.baselines.map((name) => [name, 0]));
  const skippedUnknown = Object.fromEntries(parsed.baselines.map((name) => [name, 0]));
  const warmupStart = snapshotSet.first_data_month || parsed.startMonth;
  const allMonths = monthRange(warmupStart, parsed.endMonth);
    ...
    for (const unitKey of unitUniverse(history, actual)) {
      ...
      for (const baselineName of parsed.baselines) {
        const baseline = calculateBaseline(
          baselineName, historyUnit, month, { zeroFillMissing: false },
        );
        if (!baseline) {
        ...
        if (!actualUnit?.months.has(month)) {
          if (month >= parsed.startMonth) skippedUnknown[baselineName] += 1;
          continue;
        }
        ...
        const row = {
          grain: parsed.grain, unit_id: identity.unit_id, unit_label: identity.unit_label,
          identity_validated: identity.identity_validated, month, sales: actualSales,
          baseline: baselineName, baseline_value: baseline.value,
  ...
  return { allRows, rows, skipped, skippedUnknown, months };
}
