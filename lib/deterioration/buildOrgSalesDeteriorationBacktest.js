import { buildOrgDeteriorationRuntime } from './buildOrgDeteriorationRuntime.js';
...
function warningsFor(parsed, snapshotSet, rows, semanticWarnings) {
  const final = snapshotSet.snapshots.get(parsed.endMonth);
  const warnings = new Set([...snapshotSet.context_warnings, ...semanticWarnings]);
...
export async function buildOrgSalesDeteriorationBacktest(parsed) {
  ...
  const { sourceRows, identities, observed, snapshots, backtest, evaluation } = runtime;
  const final = snapshots.snapshots.get(parsed.endMonth);
  const warnings = warningsFor(parsed, snapshots, backtest.rows, observed.warnings);
  const validation = {
  ...
  return {
    ...
    observation_semantics: observed.audit,
    identity_audit: {
    ...
    coverage: {
      ...
      skipped_unknown_actual_by_baseline: backtest.skippedUnknown,
    },
    warnings,
    validation,
  };
