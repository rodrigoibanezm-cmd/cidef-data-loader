...
function warningsFor(parsed, snapshotSet, rows) {
  ...
  if (rows.some((row) => row.baseline_value <= 0)) warnings.add('baseline zero/near-zero affects relative deviation');
...
export async function buildOrgSalesDeteriorationBacktest(parsed) {
  ...
  const evaluation = evaluateOrgCandidates(backtest.rows, parsed);
