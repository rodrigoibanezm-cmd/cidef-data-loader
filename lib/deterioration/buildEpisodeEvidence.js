...
function matchesUnitId(episode, detailUnitId) {
  return String(episode.unit_id) === String(detailUnitId);
}
...
export function buildEpisodeEvidence(rows, episodes, parsed) {
  ...
  const filtered = parsed.detailUnitId == null
    ? episodes
    : episodes.filter((episode) => matchesUnitId(episode, parsed.detailUnitId));
