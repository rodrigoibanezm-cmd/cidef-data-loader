...
function denseIndex(unit, targetMonth) {
...
function sparseIndex(unit, targetMonth) {
  const index = new Map();
  for (const [month, value] of unit?.months ?? []) {
    if (month < targetMonth) index.set(month, value);
  }
  return index;
}
...
export function calculateBaseline(name, unit, targetMonth, options = {}) {
  const index = options.zeroFillMissing === false
    ? sparseIndex(unit, targetMonth)
    : denseIndex(unit, targetMonth);
  let value = null;
