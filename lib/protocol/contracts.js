...
import {
  ApplicabilityStatus, AuthorityType, Certification, ClaimType, EvidenceClass, GoalOrigin, GoalStatus, GoalType,
...
const SEMANTIC_CONCEPTS = ['RESULT', 'COMMERCIAL_CONDITION', 'EXPECTATION', 'TEMPORAL_BEHAVIOR', 'RELATIVE_REFERENCE', 'MARKET_POSITION', 'RISK', 'CHANGE_CONTRIBUTION', 'CONCENTRATION', 'SIGNAL', 'ASSOCIATION', 'FLOW', 'LEAKAGE'];
...
function canonicalSubject(value, path) {
  ...
  requireEnum(value.entity_type, ['COMPANY', 'STORE', 'SELLER', 'BRAND', 'MODEL', 'VERSION'], `${path}.entity_type`);
...
export function validateGoalInstance(value) {
  rejectUnknown(value, ['contract_version','goal_id','question_contract_ref','universe_resolution_ref','goal_type','evidence_class','origin','status','bindings','comparison','explicit_constraints','goal_fingerprint'], '$');
  ...
  requireEnum(value.goal_type, Object.values(GoalType), '$.goal_type'); requireEnum(value.evidence_class, Object.values(EvidenceClass), '$.evidence_class'); requireEnum(value.origin, Object.values(GoalOrigin), '$.origin'); requireEnum(value.status, Object.values(GoalStatus), '$.status'); semanticBinding(value.bindings);if(Object.hasOwn(value,'comparison'))requireEnum(value.comparison,['NONE','YOY','PREVIOUS_PERIOD','EXPECTED','MARKET','PEERS'],'$.comparison');if(Object.hasOwn(value,'explicit_constraints'))requireArray(value.explicit_constraints,'$.explicit_constraints');
...
export function validateAction(value) {
  rejectUnknown(value, ['contract_version','action_id','protocol_id','issued_state_version','goal_id','candidate_id','goal_type','evidence_class','semantic_label','description','public_bindings','expected_contribution','limitations','action_fingerprint'], '$');
  requireEnum(value.contract_version, ['action.v1'], '$.contract_version'); requireString(value.action_id, '$.action_id'); requireString(value.protocol_id, '$.protocol_id'); invariant(Number.isInteger(value.issued_state_version) && value.issued_state_version > 0, 'INVALID_STATE_VERSION', '$.issued_state_version'); invariant(Object.hasOwn(value,'goal_id')!==Object.hasOwn(value,'candidate_id'),'ACTION_TARGET_XOR','$'); if(Object.hasOwn(value,'goal_id'))requireString(value.goal_id,'$.goal_id');if(Object.hasOwn(value,'candidate_id'))requireString(value.candidate_id,'$.candidate_id'); requireEnum(value.goal_type, Object.values(GoalType), '$.goal_type'); requireEnum(value.evidence_class, Object.values(EvidenceClass), '$.evidence_class'); requireString(value.semantic_label, '$.semantic_label'); requireString(value.description, '$.description');
  rejectUnknown(value.public_bindings, ['subject','measure','commercial_universe','organization_scope','period','grain','comparison','explicit_constraints'], '$.public_bindings'); requireString(value.public_bindings.subject, '$.public_bindings.subject'); requireString(value.public_bindings.measure, '$.public_bindings.measure'); requireString(value.public_bindings.commercial_universe, '$.public_bindings.commercial_universe'); requireString(value.public_bindings.organization_scope, '$.public_bindings.organization_scope'); bindingPeriod(value.public_bindings.period, '$.public_bindings.period'); requireString(value.public_bindings.grain, '$.public_bindings.grain');if(Object.hasOwn(value.public_bindings,'comparison'))requireEnum(value.public_bindings.comparison,['NONE','YOY','PREVIOUS_PERIOD','EXPECTED','MARKET','PEERS'],'$.public_bindings.comparison');if(Object.hasOwn(value.public_bindings,'explicit_constraints'))requireArray(value.public_bindings.explicit_constraints,'$.public_bindings.explicit_constraints'); stringList(value.expected_contribution, '$.expected_contribution'); requireArray(value.limitations, '$.limitations');
  rejectKeysDeep(value, PHYSICAL_FIELDS, '$'); verifyHash(value, 'action_fingerprint', ['action_id','protocol_id','goal_id','candidate_id']); return true;
}
export function createAction(draft) { return finalized(draft, 'action_fingerprint', validateAction, ['action_id','protocol_id','goal_id','candidate_id']); }
