...
const BROAD_PROPOSITIONS=Object.freeze([
  ...
  {goal_type:'TEMPORAL_TRAJECTORY',semantic_label:'REVIEW_TEMPORAL_TRAJECTORY',description:'Review the temporal trajectory.',measure:'VIN_SALES',claims:['TEMPORAL_SERIES']},
  {goal_type:'RELATIVE_PERFORMANCE',semantic_label:'COMPARE_RELATIVE_PERFORMANCE',description:'Compare relative performance.',measure:'VIN_SALES',claims:['RELATIVE_COMPARISON']},
  {goal_type:'MARKET_POSITION',semantic_label:'REVIEW_MARKET_POSITION',description:'Review market position.',measure:'MARKET_POSITION',claims:['MARKET_POSITION']},
]);
...
export function compileInitialGoals({questionContract:question,universeResolution:resolution,availabilitySnapshot:availability}){
  ...
  const candidates=BROAD_PROPOSITIONS.map((item)=>{const draft={contract_version:'initial_goal_candidate.v1',candidate_id:'pending',goal_type:item.goal_type,semantic_label:item.semantic_label,description:item.description,bindings:bindings(question,resolution,values,item.measure),evidence_class_if_selected:'REQUIRED',origin:'DEFERRED_SEMANTIC_SELECTION',expected_claim_types:item.claims,limitations:[]};const semantic=createInitialGoalCandidate(draft);draft.candidate_id=id('candidate',semantic.candidate_fingerprint);return createInitialGoalCandidate(draft);});return {goals:[],candidates};
}

export function activateCandidate(candidate,{questionContractRef,universeResolutionRef}){
  const draft={contract_version:'goal_instance.v1',goal_id:'pending',question_contract_ref:questionContractRef,universe_resolution_ref:universeResolutionRef,goal_type:candidate.goal_type,evidence_class:'REQUIRED',origin:'USER_SELECTED',status:'PENDING',bindings:clone(candidate.bindings)};const semantic=createGoalInstance(draft);draft.goal_id=id('goal',semantic.goal_fingerprint);return createGoalInstance(draft);
}
