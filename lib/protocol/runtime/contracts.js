...
export function validateProtocolState(value){
  ...
  for(const key of ['typed_signal_refs','candidate_goal_refs','transition_activation_refs'])if(Object.hasOwn(value,key))strings(value[key],`$.${key}`);
  if(Object.hasOwn(value,'assessment')){rejectUnknown(value.assessment,['iteration_count','stable','assessment_fingerprint'],'$.assessment');integer(value.assessment.iteration_count,'$.assessment.iteration_count',{minimum:1});invariant(value.assessment.stable===true,'ASSESSMENT_NOT_STABLE','$.assessment.stable');requireString(value.assessment.assessment_fingerprint,'$.assessment.assessment_fingerprint');}
  ...
  rejectUnknown(value.budget,['max_semantic_selections','semantic_selections_used','max_depth','max_transition_depth','max_active_conditional_goals','max_fixed_point_iterations'],'$.budget');integer(value.budget.max_semantic_selections,'$.budget.max_semantic_selections',{minimum:1});integer(value.budget.semantic_selections_used,'$.budget.semantic_selections_used');integer(value.budget.max_depth,'$.budget.max_depth',{minimum:1});for(const key of ['max_transition_depth','max_active_conditional_goals','max_fixed_point_iterations'])if(Object.hasOwn(value.budget,key))integer(value.budget[key],`$.budget.${key}`,{minimum:1});invariant(value.budget.semantic_selections_used<=value.budget.max_semantic_selections,'SEMANTIC_BUDGET_EXCEEDED','$.budget');
