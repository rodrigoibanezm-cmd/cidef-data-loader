...
export function validateClaimValue(type,value,path){
  requireObject(value,path);
  if(type==='OBSERVED_VALUE'){rejectUnknown(value,['amount','unit'],path);finite(value.amount,`${path}.amount`);requireString(value.unit,`${path}.unit`);return;}
  ...
  if(type==='MARKET_POSITION_VALUE'){rejectUnknown(value,['entity_value','market_value','share','rank','unit','denominator_ref'],path);for(const key of ['entity_value','market_value','share'])finite(value[key],`${path}.${key}`);finite(value.rank,`${path}.rank`,{nullable:true});for(const key of ['unit','denominator_ref'])requireString(value[key],`${path}.${key}`);return;}
