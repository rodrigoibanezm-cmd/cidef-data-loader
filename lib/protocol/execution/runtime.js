...
import { TARGET_EVIDENCE_PROJECTORS, validateTargetAnalyticalRawResult } from './projector.js';
...
export async function executeAndAdmitEvidence(state, authorization, command, {
  ...
  const executed = await executeAuthorizedMotor(state, authorization, command, {
    ...
    resultValidator:validateTargetAnalyticalRawResult, clock, execution_attempt_id,
  });
