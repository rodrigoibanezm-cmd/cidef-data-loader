...
export function validateTargetAnalyticalRawResult(raw,authorization){
  ...
  if(['MARKET/SHARE_POSITION','MARKET/SHARE_TRAJECTORY'].includes(id)){const context=raw.sharedContext;invariant(context?.context==='rvm_brand_share_trajectory_v01'&&context.validation?.ok===true,'UNEXPECTED_RESULT_SHAPE','$.raw_result.sharedContext');invariant(String(context.entity?.brandId)===String(b.subject.canonical_id)&&context.entity?.identityStatus==='RESOLVED','PHYSICAL_SUBJECT_MISMATCH','$.raw_result.sharedContext.entity');invariant(context.scope?.dateFrom===b.period.date_from&&context.scope?.dateTo===b.period.date_to&&context.scope?.organizationScope===b.organization_scope,'PHYSICAL_BINDING_MISMATCH','$.raw_result.sharedContext.scope');invariant(Array.isArray(context.monthly)&&context.monthly.length>=(id.endsWith('POSITION')?1:2),'AUTHORIZED_RESULT_POINT_MISSING','$.raw_result.sharedContext.monthly');context.monthly.forEach((row,index)=>{finite(row.entityVin,`$.raw_result.sharedContext.monthly[${index}].entityVin`);finite(row.marketSize,`$.raw_result.sharedContext.monthly[${index}].marketSize`);finite(row.marketShare,`$.raw_result.sharedContext.monthly[${index}].marketShare`);invariant(row.marketSize>0&&Math.abs(row.marketShare-row.entityVin/row.marketSize)<1e-12,'MARKET_SHARE_RECONCILIATION_FAILED',`$.raw_result.sharedContext.monthly[${index}]`);});if(id.endsWith('POSITION'))invariant(context.monthly.length===1,'AUTHORIZED_RESULT_POINT_DUPLICATE','$.raw_result.sharedContext.monthly');return raw;}
  if(id==='PRICING/HISTORY_OBSERVED'){invariant(raw.status==='OK'&&raw.commercial_condition_semantics==='PUBLISHED_COMMERCIAL_CONDITION'&&raw.provenance?.price_realized_available===false,'UNEXPECTED_RESULT_SHAPE','$.raw_result');invariant(String(raw.scope?.version_id)===String(b.subject.canonical_id)&&raw.scope?.date_from===b.period.date_from&&raw.scope?.date_to===b.period.date_to,'PHYSICAL_BINDING_MISMATCH','$.raw_result.scope');const rows=(raw.history??[]).filter(row=>row.source_status==='OK'&&row.vigencia_desde<=b.period.date_from&&(row.vigencia_hasta??b.period.date_to)>=b.period.date_to);invariant(rows.length===1,'AUTHORIZED_RESULT_POINT_MISSING','$.raw_result.history');const monetary=['precio_lista','precio_neto','precio_con_iva','bono_cidef','bono_forum','bono_mes'];monetary.forEach(key=>finite(rows[0][key],`$.raw_result.history.${key}`,{nullable:true}));invariant(monetary.some(key=>rows[0][key]!=null),'EMPTY_COMMERCIAL_CONDITION','$.raw_result.history');return raw;}
...
export function projectRvmMarketPosition(input){const {authorization,rawResult}=input;validateTargetAnalyticalRawResult(rawResult,authorization);const row=rawResult.sharedContext.monthly[0];return claimEvidence({...input,claim_type:'MARKET_POSITION_VALUE',value:{entity_value:row.entityVin,market_value:row.marketSize,share:row.marketShare,rank:null,unit:'RATIO',denominator_ref:'rvm_universe_v01:TOTAL_RVM_MARKET',market_universe:'TOTAL_RVM_MARKET',data_status:'CONSOLIDATED'},comparability:{status:'CERTIFIED',mode:'MARKET'}});}
export function projectRvmShareTrajectory(input){const {authorization,rawResult}=input;validateTargetAnalyticalRawResult(rawResult,authorization);return claimEvidence({...input,claim_type:'TRAJECTORY_SERIES',value:{unit:'RATIO',observations:rawResult.sharedContext.monthly.map(row=>({period:row.period,amount:row.marketShare,numerator:row.entityVin,denominator:row.marketSize})),common_basis:'CERTIFIED_RVM_MARKET_SHARE',minimum_observations:2,market_universe:'TOTAL_RVM_MARKET',data_status:'CONSOLIDATED'},comparability:{status:'CERTIFIED',mode:'NONE'}});}
export function projectPricingObservedCondition(input){const {authorization,rawResult}=input;validateTargetAnalyticalRawResult(rawResult,authorization);const b=authorization.semantic_binding,row=rawResult.history.find(item=>item.source_status==='OK'&&item.vigencia_desde<=b.period.date_from&&(item.vigencia_hasta??b.period.date_to)>=b.period.date_to);return claimEvidence({...input,claim_type:'OBSERVED_COMMERCIAL_CONDITION',value:{semantic_basis:'PUBLISHED_COMMERCIAL_CONDITION',data_status:'AVAILABLE',validity:{date_from:row.vigencia_desde,date_to:row.vigencia_hasta},price_episode_id:String(row.price_episode_id),price_version_id:String(row.price_version_id),published_list_price:row.precio_lista,published_net_price:row.precio_neto,published_price_with_tax:row.precio_con_iva,bonus_cidef:row.bono_cidef,bonus_forum:row.bono_forum,bonus_month:row.bono_mes,unit:'CLP',source_status:row.source_status,source_files:[...(row.source_files??[])]},comparability:{status:'NOT_REQUIRED',mode:'NONE'}});}
...
export const TARGET_EVIDENCE_PROJECTORS = Object.freeze({
  ...
  'protocol.evidence.rvm_market_position.v1': projectRvmMarketPosition,
  'protocol.evidence.rvm_share_trajectory.v1': projectRvmShareTrajectory,
  'protocol.evidence.pricing_observed_condition.v1': projectPricingObservedCondition,
});
