...
export function validateUniverseResolutionRequest(value){
  ...
  rejectUnknown(value.subject,['expression','type_constraints','reference_kind'],'$.subject');requireString(value.subject.expression,'$.subject.expression');requireArray(value.subject.type_constraints,'$.subject.type_constraints',{nonEmpty:true});value.subject.type_constraints.forEach((item,index)=>requireEnum(item,['COMPANY','STORE','SELLER','BRAND','MODEL'],`$.subject.type_constraints[${index}]`));requireEnum(value.subject.reference_kind,REFERENCES,'$.subject.reference_kind');
  rejectUnknown(value.requested_scope,['commercial_universe','organization_scope','market_universe','geography'],'$.requested_scope');requireEnum(value.requested_scope.commercial_universe,['COMPANY','OWN_STORES','DEALERS'],'$.requested_scope.commercial_universe');axis(value.requested_scope.organization_scope,'$.requested_scope.organization_scope');axis(value.requested_scope.market_universe,'$.requested_scope.market_universe');axis(value.requested_scope.geography,'$.requested_scope.geography');period(value.requested_period,'$.requested_period');requireString(value.requested_grain,'$.requested_grain');requireArray(value.explicit_constraints,'$.explicit_constraints');value.explicit_constraints.forEach((item,index)=>validateExplicitConstraint(item,`$.explicit_constraints[${index}]`));
