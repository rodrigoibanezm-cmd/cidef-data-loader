...
const ALLOWED_KEYS = new Set(['start_month', 'end_month', 'output_mode', 'milestone_days', 'percentiles']);
...
function parsePercentiles(input) {
  if (!Object.prototype.hasOwnProperty.call(input, 'percentiles')) return null;
  const value = input.percentiles;
  if (!Array.isArray(value) || value.length === 0) {
    throw new Error('percentiles must be a non-empty array');
  }
  if (value.length > 10) {
    throw new Error('percentiles must contain at most 10 values');
  }
  if (value.some((item) => typeof item !== 'number' || !Number.isFinite(item))) {
    throw new Error('percentiles must contain only finite numbers');
  }
  if (value.some((item) => item < 0 || item > 100)) {
    throw new Error('percentiles must contain values between 0 and 100');
  }
  if (new Set(value).size !== value.length) {
    throw new Error('percentiles must not contain duplicates');
  }
...
export function parseHistoryRange(input = {}, now = new Date()) {
  ...
  const percentiles = parsePercentiles(input);
  if (outputMode === 'DAILY' && percentiles !== null) {
    throw new Error('percentiles is only supported with output_mode=MONTHLY_MILESTONES');
  }
