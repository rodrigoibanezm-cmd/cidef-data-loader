...
function forecastRow(row, grain, learned, trainingCount, trainingLastMonth) {
  const base = {
    grain,
    target_month: row.target_month,
    day_of_month: row.day_of_month,
    training_observations: trainingCount,
    training_last_month: trainingLastMonth,
...
export function buildWalkForwardForecasts(
  ...
  for (const byMonth of grouped.values()) {
    const prior = [];
    let trainingLastMonth = null;
    ...
    for (const month of months) {
      ...
      if (month >= startMonth && month <= endMonth) {
        for (const row of rows) {
          output.push(forecastRow(row, grain, learned, prior.length, trainingLastMonth));
        }
      }
      for (const row of rows) {
