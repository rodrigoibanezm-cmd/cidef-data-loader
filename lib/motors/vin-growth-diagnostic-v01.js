...
function buildRvmContext(rvm, periods) {
  const organizationCoverage = rvm?.coverage?.organizationCoverage ?? null;
  const organizationResolved = organizationCoverage?.scope === 'CIDEF' && organizationCoverage?.state === 'RESOLVED';
  const sourceEvaluable = !hasNoCoverage(rvm) && organizationResolved;
  ...
  return {
    ...
    organization_resolution_state: organizationCoverage?.state ?? 'NOT_EVALUABLE',
    organization_coverage: organizationCoverage,
...
export function assembleVinGrowthDiagnostic({ scope, periods, ventas, rvm, crmConversion, crmSold }) {
  ...
  return {
    ...
    warnings: rvmContext.evaluable ? [] : [`RVM_ORGANIZATION_${rvmContext.organization_resolution_state}`],
  };
}
