...
function sideInfo(side, leftCols, rightCols, context = 'field') {
...
export async function run(input = {}) {
  ...
  const groupSql = groups.map((g, i) => {
    if (!g || typeof g !== 'object' || Array.isArray(g)) {
      throw new Error(`group_by[${i}] must be an object with side and column`);
    }
    if (!g.column) throw new Error(`group_by[${i}].column is required`);
    const info = sideInfo(g.side, leftCols, rightCols, `group_by[${i}]`);
    if (!info.columns.has(g.column)) throw new Error(`group_by[${i}].column is not present on ${g.side} table`);
  ...
  const metricSql = metrics.map((m, i) => {
    if (!m || typeof m !== 'object' || Array.isArray(m)) throw new Error(`metrics[${i}] must be an object`);
    if (!AGGS.has(m.op)) throw new Error(`metrics[${i}].op is invalid`);
    const as = m.as || `${m.op}_${i + 1}`;

    if (m.op === 'count' && !m.column) return { expression: 'COUNT(*)', as };
    if (!m.column) throw new Error(`metrics[${i}].column is required for ${m.op}`);

    const info = sideInfo(m.side, leftCols, rightCols, `metrics[${i}]`);
    if (!info.columns.has(m.column)) throw new Error(`metrics[${i}].column is not present on ${m.side} table`);
