...
function latestCrmCte() {
  return `crm_latest AS (
      SELECT DISTINCT ON ("ID") *
      FROM public."CRM_Cidef_raw"
      WHERE coalesce("ID",'')<>''
...
async function refreshBridge() {
  await queryDb(`
    ...
    SELECT
      ...
      CASE WHEN u.deal_id IS NOT NULL THEN 'RUT_STORE_14D_UNIQUE_LATEST_CRM' ELSE NULL END,
      u.day_gap,
      cv.vin,
...
export async function run() {
  ...
  const commercialMaster = await commercialOperationMaster.run();
  return {
    status: 'refreshed',
    table: TABLE,
    match_rule: `RUT + CIDEF_STORE + +/-${WINDOW_DAYS}D + UNIQUE_LATEST_CRM_DEAL`,
    ...
    commercial_operation_master: commercialMaster,
