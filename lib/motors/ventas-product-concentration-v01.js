...
export function calculateProductConcentration(context, modelCatalog, parsed) {
  if (context.cutoff_month !== parsed.endMonth) throw new Error('ventas product context cutoff mismatch');
  ...
  const validationsOk = context.ventas_validation?.ok === true
...
export async function ventasProductConcentrationV01(input = {}) {
  ...
  const [context, modelCatalog] = await Promise.all([
    buildVentasProductContext({ cutoffMonth: parsed.endMonth }),
    loadModelCatalog(),
  ]);
  return calculateProductConcentration(context, modelCatalog, parsed);
}
