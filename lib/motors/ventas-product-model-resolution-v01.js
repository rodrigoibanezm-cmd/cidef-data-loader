...
export async function ventasProductModelResolutionV01(input = {}) {
  ...
  const unresolved = sales.length - resolved.length - ambiguousCount;
  ...
  return {
    ...
    status: ambiguousCount === 0 && unresolved === 0 ? 'ok' : 'warning',
    ...
    coverage: {
      ...
      ambiguous_model_sales: ambiguousCount,
    ...
    validation: {
      ...
      arithmetic_reconciles: resolved.length + ambiguousCount + unresolved === sales.length,
    ...
    warnings: [
      ...(ambiguousCount ? ['Some recognized sales remain ambiguous at model identity'] : []),
