...
export async function run(input = {}) {
  ...
  const [bounds] = await sql.query(`SELECT
    ...
    ${comparison === 'rolling' ? monthExpr(`- interval '${months} month'`) : monthExpr(`- interval '1 year' - interval '${months - 1} month'`)} AS compare_start,
    ${comparison === 'rolling' ? monthExpr(`- interval '${months} month' + interval '${months - 1} month'`) : monthExpr(`- interval '1 year'`)} AS compare_end
  `, [endDate]);
  ...
  const summary = current.map(r => {
    ...
    return { ...r, comparacion_penetracion_pct: p?.penetracion_pct ?? null,
      delta_pp: p ? Number(r.penetracion_pct) - Number(p.penetracion_pct) : null };
  });

  return { universe, segment, months, comparison,
