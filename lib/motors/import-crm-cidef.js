...
import { run as refreshForumCommercialBridge } from './forum-commercial-bridge-v01.js';
...
async function maybeRefreshForumBridge(input) {
  if (input.skipForumBridge === true) return null;
  const forumExists = await queryDb(
    `SELECT 1 FROM information_schema.tables WHERE table_schema='public' AND table_name='forum_raw' LIMIT 1`,
  );
  if (!forumExists.length) return null;
  return refreshForumCommercialBridge();
}
...
export async function run(input = {}) {
  ...
  const forumBridge = await maybeRefreshForumBridge(input);
