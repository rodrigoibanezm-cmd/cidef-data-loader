...
export function calculateVentasMonthlyDedupSensitivity(rows, input) {
  ...
  for (const row of rows) {
    ...
    if (!current) {
      byVin.set(vin, { count: 1, first: parsed, last: parsed });
      continue;
    }
    current.count += 1;
    if (parsed.date < current.first.date) current.first = parsed;
    if (parsed.date > current.last.date) current.last = parsed;
  }

  if (nullFechaRows) warnings.push(`${nullFechaRows} rows have null/blank fecha_factura and were not assignable to a month`);
  if (unparseableRows) warnings.push(`${unparseableRows} rows have unparseable fecha_factura and were not assignable to a month`);
  ...
  for (const { count, first, last } of byVin.values()) {
    if (count === 1) vinsSingleOccurrence += 1;
    else vinsMultipleOccurrences += 1;
    if (first.month === last.month) vinsSameFirstLastMonth += 1;
    else {
      vinsWithDifferentFirstLastMonth += 1;
      const key = `${first.month}|${last.month}`;
      transfers.set(key, (transfers.get(key) || 0) + 1);
    }
    firstCounts.set(first.month, (firstCounts.get(first.month) || 0) + 1);
    lastCounts.set(last.month, (lastCounts.get(last.month) || 0) + 1);
  }
  ...
  for (const row of monthly) {
    ...
    const priorMonth = `${year - 1}-${String(month).padStart(2, '0')}`;
    const priorIndex = monthIndex.get(priorMonth);
    if (priorIndex == null) continue;
    const prior = monthly[priorIndex];
  ...
  const maxPctRow = monthly.reduce((best, row) => {
    if (row.delta_pct == null) return best;
    return !best || row.delta_pct > best.delta_pct ? row : best;
  }, null);
  ...
  const expectedUnitsAssignable = byVin.size + [...nullVinMonths.values()].reduce((sum, value) => sum + value, 0);
  const expectedUnitsRaw = byVin.size + nullVinRows;
  ...
  return {
    ...
    parser: {
      accepted_format: 'MM/DD/YY H:MI, MM/DD/YYYY H:MI, optional seconds',
      two_digit_year_policy: '2000 + YY',
      timezone_policy: 'UTC calendar parsing; time does not affect month assignment',
    },
    coverage: {
...
export async function ventasMonthlyDedupSensitivityV01(input = {}) {
  ...
  const rows = await sql.query(
    `SELECT nro_vin_chasis, fecha_factura
     FROM ventas_raw`,
  );
