...
import { buildRvmBrandShareTrajectory } from '../rvm-brand-share-trajectory/buildRvmBrandShareTrajectory.js';

export const ENGINE_NAME = 'competitive_share_trajectory_v01';
export const ENGINE_VERSION = '0.2';

export async function competitiveShareTrajectoryV01(input = {}, dependencies = {}) {
  if (input?.entity != null) {
    const buildBrandTrajectory = dependencies.buildBrandTrajectory ?? buildRvmBrandShareTrajectory;
    const context = await buildBrandTrajectory(input);
    return {
      engine: ENGINE_NAME,
      version: ENGINE_VERSION,
      status: context.validation.ok ? (context.warnings.length ? 'warning' : 'ok') : 'warning',
      policy: {
        dependency: 'rvm_longitudinal_context_v01 over rvm_universe_v01',
        entity_scope: 'BRAND resolved exactly by canonical MASTER identity',
        share_denominator: 'certified total RVM market units for the same period; organization_scope only filters the numerator',
        reconstruction: 'none; brand membership and RVM identity are backend responsibilities',
      },
