...
export async function ventasProductModelResolutionAuditV01(input = {}) {
  ...
  for (const sale of direct) {
    ...
    if (directModel && candidateModel && directModel !== candidateModel) {
      ...
      ambiguous += 1;
      ambiguousSku.set(norm(sale.producto_sku), {
      ...
      ambiguousSku.set(norm(sale.producto_sku), {
        ...
        direct_modelo_id: directModel,
        rvm_modelo_id: candidateModel,
        distinct_rvm_model_ids: Number(evidence?.distinct_model_ids || 0),
        ambiguous_evidence_rows: Number(evidence?.ambiguous_evidence_rows || 0),
      });
    } else {
      unresolved += 1;
  ...
  return {
    ...
    sku_evidence: {
      ...
      cross_method_conflict_skus: [...ambiguousSku.values()],
    },
    ...
    validation: {
      ...
      no_candidate_multi_destination_sku: evidenceRows.every((row) => Number(row.distinct_model_ids || 0) <= 1 || candidateStatus(row) === 'AMBIGUOUS'),
    },
  };
}
