...
const hasFingerprint = (item) => FP_FIELDS.some((field) => item[field]);
const conflicts = (existing, item) => FP_FIELDS.some((field) =>
  existing[field] && item[field] && String(existing[field]).trim().toUpperCase() !== String(item[field]).trim().toUpperCase());
...
async function getPriceVersionId(item, cache) {
  ...
  let id;
  if (!found.length) {
    const status = hasFingerprint(item) ? 'complete' : 'incomplete';
    ...
    const status = conflict ? 'conflict' : (hasFingerprint(item) ? 'complete' : 'incomplete');
    await queryDb(
...
export async function run() {
  ...
  try {
    ...
    const fingerprintConflicts = (await queryDb(`SELECT count(*)::int AS n FROM price_versions WHERE fingerprint_status='conflict'`))[0].n;
    const fingerprintIncomplete = (await queryDb(`SELECT count(*)::int AS n FROM price_versions WHERE fingerprint_status='incomplete'`))[0].n;
    return { runId, filesLoaded: files.length, rowsStaged: records.length,
      ...
      ...result, fingerprintConflicts, fingerprintIncomplete, elapsedMs: Date.now() - startedAt };
  } catch (error) {
