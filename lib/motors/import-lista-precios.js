...
const DEFAULT_RESUME_BATCH_SIZE = 250;
const MIN_RESUME_BATCH_SIZE = 50;
const MAX_RESUME_BATCH_SIZE = 500;
...
function resolveResumeBatchSize(value) {
  const parsed = Number(value);
  if (!Number.isFinite(parsed)) return DEFAULT_RESUME_BATCH_SIZE;
  return Math.max(MIN_RESUME_BATCH_SIZE, Math.min(MAX_RESUME_BATCH_SIZE, Math.trunc(parsed)));
}

async function loadStagedBatch(runId, batchSize) {
...
export async function run(input = {}) {
  ...
  if (resumeRunId) {
    const batchSize = resolveResumeBatchSize(input?.resume_batch_size);
    try {
      const staged = await loadStagedBatch(resumeRunId, batchSize);
      const normalized = staged.records.map(normalizePriceRecord).filter(Boolean);
      const result = await persistCanonical(normalized);

      await queryDb(
        `DELETE FROM ${STAGING_TABLE} WHERE run_id = $1 AND id <= $2`,
        [resumeRunId, staged.lastId],
      );

      const remainingRows = await queryDb(
        `SELECT count(*)::int AS n FROM ${STAGING_TABLE} WHERE run_id = $1`,
        [resumeRunId],
      );
