...
async function loadStagedRecords(runId) {
  const rows = await queryDb(
    `SELECT payload, source_file FROM ${STAGING_TABLE} WHERE run_id = $1 ORDER BY id`,
    [runId],
  );
...
export async function run(input = {}) {
  ...
  if (resumeRunId) {
    runId = resumeRunId;
    const staged = await loadStagedRecords(runId);
  ...
  try {
    ...
    return { runId, resumed, filesLoaded, rowsStaged: records.length,
