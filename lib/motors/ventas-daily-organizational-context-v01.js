...
import { commercialDayForDate, commercialMonthForDate } from '../ventas/commercialMonth.js';
...
import { observedStoreKeysUnique } from '../ventas-org/observedStoreKeysUnique.js';
...
export function calculateVentasDailyOrganizationalContext(context, identityMaps, { cutoffDate }) {
  ...
  const month = commercialMonthForDate(cutoffDate);
  const dayOfMonth = commercialDayForDate(cutoffDate);
  const recognizedSales = context.recognizedSales || [];
  const events = enrichRecognizedSales(recognizedSales, identityMaps);
  ...
  const storeKeysUnique = observedStoreKeysUnique(recognizedSales, identityMaps?.stores, month);
  const validations = {
  ...
  return {
    ...
    policy: {
      ...
      commercial_month: 'calendar day 02 through next calendar month day 01 inclusive',
    ...
    as_of: { cutoff_date: cutoffDate, month, day_of_month: dayOfMonth },
