...
async function loadInternalCommercialEvidence(sql = customGptDb()) {
  return sql.query(`
    ...
    exact_candidates AS MATERIALIZED (
      ...
      UNION
      SELECT DISTINCT e.sku_norm,m.modelo_id,'MASTER_VERSION_SKU_EXACT'::text AS method
      FROM eligible e
      JOIN marcas_master_v01 ma ON ma.nombre_normalizado=e.brand_norm
      JOIN modelos_master_v01 m ON m.marca_id=ma.marca_id
      JOIN versiones_master_v01 v ON v.modelo_id=m.modelo_id AND v.nombre_normalizado=e.sku_norm
      UNION
      SELECT DISTINCT e.sku_norm,m.modelo_id,'MASTER_VERSION_COMMERCIAL_EXACT'::text AS method
      FROM eligible e
      JOIN marcas_master_v01 ma ON ma.nombre_normalizado=e.brand_norm
      JOIN modelos_master_v01 m ON m.marca_id=ma.marca_id
      JOIN versiones_master_v01 v ON v.modelo_id=m.modelo_id AND v.nombre_normalizado=e.commercial_norm
      UNION
...
export async function ventasProductInternalModelResolutionAuditV01(input = {}) {
  ...
  return {
    ...
    policy:{
      ...
      candidate_rule:'ventas_raw articulo == notas_venta_raw modelo technical SKU; require one stable modelo_comercial + brand per SKU; accept only exact normalized equality to MASTER model name, MASTER version name, or certified RVM model alias within canonical brand',
