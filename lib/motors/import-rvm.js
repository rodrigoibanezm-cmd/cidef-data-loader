...
async function processFile(filePath, file, startedAt) {
...
export async function run(input = {}) {
  ...
  const firstPath = path.join(os.tmpdir(), `${files[0].id}.xlsx`);
  if (input.reset === true || !(await stagingExists())) {
    await downloadFileToPath(files[0].id, firstPath);
    try {
      const { columns } = await readHeaders(firstPath);
      await resetRvmImport(['source_year', 'source_file', 'source_sheet', 'source_row', ...columns]);
    } finally {
      await fs.unlink(firstPath).catch(() => {});
    }
  }
  ...
  for (const file of files) {
    ...
    try {
      const result = await processFile(filePath, file, startedAt);
