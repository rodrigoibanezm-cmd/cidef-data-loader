...
function requireReadyAuthority(authority){
  if(authority?.ready!==true) throw Object.assign(new Error('RESOLUTION_NOT_READY'),{code:'RESOLUTION_NOT_READY'});
  if(!authority.period?.type || !authority.period?.date_from || !authority.period?.date_to) throw Object.assign(new Error('RESOLUTION_TEMPORAL_AUTHORITY_REQUIRED'),{code:'RESOLUTION_TEMPORAL_AUTHORITY_REQUIRED'});
  if(!authority.entity) throw Object.assign(new Error('RESOLUTION_ENTITY_AUTHORITY_REQUIRED'),{code:'RESOLUTION_ENTITY_AUTHORITY_REQUIRED'});
}
function materializeIntent(raw,authority){
  ...
  value.period=value.period??{type:authority.period.type,date_from:authority.period.date_from,date_to:authority.period.date_to};
...
export async function analyzeIntent(request={},options={}){
  ...
  requireReadyAuthority(authority);
