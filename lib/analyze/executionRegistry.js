...
function executionKey(execution) { return execution == null ? null : JSON.stringify(stableValue(execution)); }

const CONTEXT_EVIDENCE_ASSOCIATION = Object.freeze({
  SALES_CONTEXT: 'CURRENT_RESULT',
  MARKET_CONTEXT: 'MARKET_REFERENCE',
  CRM_CONTEXT: 'CRM_CONTEXT_SIGNAL',
});
...
export function resolveExecutionRequirement(requirement, plan, authority){
  ...
  switch(requirement.type){
    case 'CURRENT_RESULT':
      if(p.type==='CURRENT_MTD' && e.type==='COMPANY') {
        // The live forecast computes cidef_propio, not the full COMPANY universe.
        // Missing certified support must not become own-store evidence labelled COMPANY.
        if(cu!=='OWN_STORES') return null;
    ...
    case 'CLOSE_EXPECTATION':
      if(p.type!=='CURRENT_MTD' || cu!=='OWN_STORES') return null;
      return {domain:'SALES',capability:'CURRENT_MONTH_CLOSE_FORECAST',input:{cutoff_date:p.date_to}};
...
export function resolveContextRequirement(contextType, plan, authority){
  ...
  if(contextType==='MARKET_CONTEXT') {
    const e=authority.entity;
    const entity=['BRAND','MODEL'].includes(e?.type) ? {type:e.type,canonical_id:e.canonical_id,display_name:e.display_name} : null;
    if(!entity?.canonical_id) return {key:'market',execution:null};
    return {key:'market',execution:{domain:'MARKET',capability:'COMPETITIVE_CONTEXT',input:{date_from:p.date_from,date_to:p.date_to,entity}}};
  }
...
export function buildInvestigationSteps(plan, authority) {
  ...
  for (const requirement of plan.evidence_requirements) {
    if (requirement.requirement !== 'REQUIRED' || !requiredEvidence.has(requirement.type)) continue;
    const execution = resolveExecutionRequirement(requirement, plan, authority);
    const key = executionKey(execution);
    let investigation = key == null ? null : byExecution.get(key);
    if (!investigation) {
      investigation = {
        step_id: `investigation_${investigations.length + 1}`,
        requirements: [],
        execution,
        contexts: [],
      };
      investigations.push(investigation);
      if (key != null) byExecution.set(key, investigation);
    }
    investigation.requirements.push(requirement);
  }

  const requiredContextSteps = buildContextSteps(plan, authority)
    .filter((step) => step.requirement.requirement === 'REQUIRED'
      && requiredContexts.has(step.requirement.type));
  for (const contextStep of requiredContextSteps) {
    const preferredEvidence = CONTEXT_EVIDENCE_ASSOCIATION[contextStep.requirement.type];
    const target = investigations.find((step) =>
      step.requirements.some((requirement) => requirement.type === preferredEvidence))
      ?? investigations[0];
    if (!target) continue;
    target.contexts.push(contextStep);
  }
