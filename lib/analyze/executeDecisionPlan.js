...
import { materializeMarketContextExecution } from './marketContextExecutionAdapter.js';
import { sanitizePublicValue } from './publicSanitizer.js';
import { projectEvidence } from './evidenceProjection.js';
import { projectPublicRequirement } from './publicProjectionRegistry.js';

async function defaultExecutor(spec) {
  const execution = await materializeMarketContextExecution(spec);
...
async function executeEvidence(investigation, executor, plan, intent, internalTrace) {
  ...
  try {
    ...
    const projection = projectEvidence({ investigation, plan, intent, raw });
    internalTrace.push({
      step_id: investigation.step_id,
      requirements: investigation.requirements.map((requirement) => requirement.type),
    ...
    if (projection.status !== 'AVAILABLE') {
      return {
        ...base,
        status: 'NOT_EVALUABLE',
        reason_code: projection.reason_code ?? 'EVIDENCE_PROJECTION_NOT_EVALUABLE',
        coverage: projection.coverage ?? { status: 'NOT_EVALUABLE' },
        ...(projection.evidence === undefined ? {} : { data: sanitizePublicValue(projection.evidence) }),
      };
    }
    return {
      ...base,
      status: 'AVAILABLE',
      coverage: projection.coverage ?? { status: 'AVAILABLE' },
      data: sanitizePublicValue(projection.evidence),
    };
  } catch (error) {
    internalTrace.push({
      step_id: investigation.step_id,
...
async function executeContexts(investigation, executor, plan, intent, internalTrace) {
  ...
  for (const step of investigation.contexts) {
    ...
    try {
      ...
      const projection = projectPublicRequirement({
        channel: 'CONTEXT',
      ...
      if (projection.status !== 'AVAILABLE') {
        items.push({
          kind: contextKind(step.requirement.type),
          status: 'NOT_EVALUABLE',
          reason_code: projection.reason_code ?? 'PUBLIC_CONTRACT_INSUFFICIENT_INPUT',
          ...(projection.data === undefined ? {} : { data: sanitizePublicValue(projection.data) }),
        });
        internalTrace.push({
          step_id: step.step_id,
          context_type: step.requirement.type,
          ...
          status: 'NOT_EVALUABLE',
          error_code: projection.reason_code ?? 'PUBLIC_CONTRACT_INSUFFICIENT_INPUT',
        });
        continue;
      }
      items.push({
        kind: contextKind(step.requirement.type),
        status: 'AVAILABLE',
        data: sanitizePublicValue(projection.data),
      });
      internalTrace.push({
        step_id: step.step_id,
        context_type: step.requirement.type,
...
export async function executeDecisionPlan(plan, authority, intent, options = {}) {
  ...
  const contextPayload = await executeContexts(investigation, executor, plan, intent, internalTrace);
