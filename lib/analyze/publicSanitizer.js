const FORBIDDEN_KEYS = new Set(['domain','capability','motor','action','sql','table','dependency','dependencies','target_model_ids','universe','universe_name','universe_version','engine','question_family','policy','execution','execution_trace','drill_policy','source_table','source_file']);
...
export function containsForbiddenArchitecture(value){
  ...
  return ['"capability"','"motor"','target_model_ids','"sql"','"question_family"','ventas_universe_v01','rvm_universe_v01','crm_universe_v01','commercial_operation_universe_v01'].some(x=>s.includes(x));
}
