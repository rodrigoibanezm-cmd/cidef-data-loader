...
function validProposition(value) {
  ...
  return !!value && typeof value==='object' && !Array.isArray(value)
    && value.type==='COMPARATIVE_TEMPORAL_SUPPORT'
    && value.comparison==='YOY'
    && value.reference?.type==='SAME_PERIOD_PREVIOUS_YEAR'
    && ['AGGREGATE_ONLY','AGGREGATE_AND_MONTHLY_SUPPORT'].includes(value.evaluation?.mode)
    && Object.keys(value).every((key)=>['type','comparison','reference','evaluation'].includes(key))
    && Object.keys(value.reference??{}).every((key)=>key==='type')
    && Object.keys(value.evaluation??{}).every((key)=>key==='mode');
}
...
export function validateDecisionPlan(plan){
  ...
  if(!Array.isArray(plan.evidence_requirements)||plan.evidence_requirements.some(x=>!EVIDENCE_REQUIREMENTS.includes(x.type)||!['REQUIRED','OPTIONAL'].includes(x.requirement)||!validProposition(x.proposition))) throw Object.assign(new Error('INVALID_EVIDENCE_REQUIREMENTS'),{code:'INVALID_EVIDENCE_REQUIREMENTS'});
