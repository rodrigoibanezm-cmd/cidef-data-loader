...
function lastRowsByMonth(rows) {
  const out = new Map();
  for (const row of rows) {
    const current = out.get(row.target_month);
    if (!current || row.cutoff_date > current.cutoff_date) out.set(row.target_month, row);
  }
  return out;
}
...
export function validateBacktestObservations(result) {
  ...
  const companyLast = lastRowsByMonth(company);
  const storeLast = lastRowsByMonth(stores);
  ...
  const validations = {
    ...
    company_month_end_equals_close: [...companyLast.values()].every((r) => r.observed_to_date === r.actual_close),
    store_month_end_equals_close: [...storeLast.values()].every((r) => r.observed_to_date === r.actual_close),
  ...
  return {
    validations,
    ok: Object.values(validations).every(Boolean),
  };
}
