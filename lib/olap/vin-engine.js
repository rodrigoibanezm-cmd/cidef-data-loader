...
function executeTemporalBoundary(input, sourceRows, dealerRows) {
  ...
  if (input.universe.type === 'EVENT_POPULATION') {
    const eventColumn = VIN_CUBE.timeRoles[input.universe.event];
    universe = eligible.filter((r) => parseSourceDate(r[eventColumn]).status === 'parsed');
  }
  ...
  const temporal = auditTemporal(filtered.map((r) => r[timeColumn]), parseSourceDate);
  const used = filtered
  ...
  const selected = dates.length
    ? (input.boundary === 'MIN'
      ? new Date(Math.min(...dates.map((d) => d.getTime())))
      : new Date(Math.max(...dates.map((d) => d.getTime()))))
    : null;
  const boundary = selected ? dateKey(selected, input.time.grain) : null;
  ...
  checks.push({ name:'Temporal Parse Audit', status:temporal.status, role:input.time.role, details:temporal });
  ...
  checks.push({
    ...
    parsed_rows_considered:used.length,
    result:boundary,
  });
  const warnings = [];
  ...
  return {
    ...
    lineage:input.options?.include_lineage === false ? undefined : {
      ...
      normalizations:['VIN:TRIM','text:TRIM+control-whitespace+UPPER'],
      identity_masters:[],
    },
  };
}
...
export function executeVinOlap(input, sourceRows, dealerRows = []) {
  ...
  if ((input.operation || 'AGGREGATE') === 'TEMPORAL_BOUNDARY') {
    return executeTemporalBoundary(input, sourceRows, dealerRows);
  }
  const vinAudit = auditVinUniverse(sourceRows, (r) => r[VIN_CUBE.fact.key]);
  if (vinAudit.status === 'FAIL') return fail('VIN_GRAIN_VIOLATION', 'duplicate VIN detected', input);
