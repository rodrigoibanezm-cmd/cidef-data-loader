...
function validateQuery(input) {
  ...
  if (input.time?.from != null && !parseIsoDate(input.time.from)) return ['INVALID_QUERY', 'time.from must be YYYY-MM-DD'];
  if (input.time?.to != null && !parseIsoDate(input.time.to)) return ['INVALID_QUERY', 'time.to must be YYYY-MM-DD'];
...
function dimValue(row, dim, dealerMap) {
  ...
  if (dim.name === 'dealer_sale' && dim.level === 'canonical') return normalized ? (dealerMap.get(normalized) ?? '__UNMATCHED__') : '__MISSING__';
  return null;
}
...
export function executeVinOlap(input, sourceRows, dealerRows = []) {
  ...
  if (input.time) {
    ...
    filtered = filtered.filter((r) => {
      ...
      if (p.status !== 'parsed') { invalidRequired += 1; return false; }
  ...
  for (const row of filtered) {
    ...
    for (const d of input.dimensions || []) {
      const v = dimValue(row, d, dealerMap); out[d.as || d.name] = v == null ? '__MISSING__' : v; keyParts.push(out[d.as || d.name]);
    }
    if (input.time?.grain) {
      const tk = dateKey(parseSourceDate(row[VIN_CUBE.timeRoles[input.time.role]]).date, input.time.grain); out.time = tk; keyParts.push(tk);
    }
  ...
  const allRows = [...groups.values()].map((g) => {
    ...
    for (const dm of input.derived_metrics || []) {
      const vals = r.__aging; const a = dm.as || `${dm.name}_${dm.aggregation.toLowerCase()}`;
  ...
  const ur = reconcileUniverse({ source: sourceRows.length, eligible: eligible.length, filtered: universe.length, used: filtered.length, excludedIneligible, excludedByFilter: excludedByUniverse, excludedInvalid: excludedByFilter });
