...
function dimensionExpression(dim, alias = 'i') {
  ...
  if (dim.name === 'dealer_sale' && dim.level === 'canonical') return 'd.dealer_id::text';
...
function filterSql(input, values) {
  ...
  for (const f of input.filters || []) {
    ...
    const op = f.op;
    if (op === 'is_null') { parts.push(`${expr} IS NULL`); continue; }
    if (op === 'not_null') { parts.push(`${expr} IS NOT NULL`); continue; }
    if (op === 'in' || op === 'not_in') {
      if (!Array.isArray(f.value) || !f.value.length) throw Object.assign(new Error('invalid in filter'), { code: 'INVALID_QUERY' });
      const ps = f.value.map((v) => push(values, v)); parts.push(`${expr} ${op === 'in' ? 'IN' : 'NOT IN'} (${ps.join(',')})`); continue;
    }
    if (op === 'between') {
    ...
    if (!map[op]) throw Object.assign(new Error('unsupported filter op'), { code: 'INVALID_QUERY' });
    const numeric = VIN_CUBE.dimensions[f.field.name].type === 'numeric';
    parts.push(`${expr} ${map[op]} ${push(values, f.value)}${numeric ? '::numeric' : ''}`);
  }
  return parts;
}
...
export function buildVinSqlPlan(input) {
  ...
  return {
    ...
    total: `SELECT COUNT(*)::int AS total FROM ${from} WHERE ${where}`,
    ...
    universeCount: `SELECT COUNT(*)::int universe_rows FROM ${from} WHERE ${[eligibleWhere(), ...filterSql({ ...input, time: null, filters: [] }, [])].join(' AND ')}`,
