...
export async function run(input = {}) {
  ...
  const sql = db();
  const [sourceAuditRows, duplicateRows, eligibleRows, universeRows, totalRows, groupCountRows, groupedTotalRows, resultRows, temporalRows] = await Promise.all([
    sql.query(plan.sourceAudit),
    sql.query(plan.duplicateAudit),
    sql.query(plan.eligibleCount),
    sql.query(plan.universeCount),
    sql.query(plan.total, plan.values),
  ...
  const filtered = Number(totalRows[0]?.total || 0);
  const groupedTotal = Number(groupedTotalRows[0]?.total || 0);
  if (groupedTotal !== filtered) return fail('AGGREGATION_RECONCILIATION_FAILURE', 'group totals do not match filtered universe', input);

  const sourceRows = Number(src?.source_rows || 0);
  const excludedIneligible = sourceRows - eligible;
  const excludedByFilter = eligible - filtered;
  if (sourceRows !== eligible + excludedIneligible || eligible !== filtered + excludedByFilter) {
  ...
  checks.push({ name:'Universe Reconciliation', status:'PASS', equations:{ source:true, eligible:true, filtered:true } });
  checks.push({ name:'Aggregation Reconciliation', status:'PASS', expected:filtered, actual:groupedTotal });
  ...
  return {
    ...
    result:{ rows:resultRows, totals:input.options?.include_totals === false ? {} : { [measureAlias]:filtered }, rows_returned:resultRows.length, has_more:plan.offset + resultRows.length < groups },
    coverage:input.options?.include_coverage === false ? undefined : { source_rows:sourceRows, eligible_vin:eligible, universe_rows:universe, filtered_rows:filtered, normalized_rows:filtered, used_rows:filtered, excluded:[{ reason:'ineligible_vin', rows:excludedIneligible },{ reason:'filters_or_time', rows:Math.max(0, universe-filtered) }] },
