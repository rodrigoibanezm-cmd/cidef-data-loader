...
async function runTemporalBoundary(sql, input, plan) {
  const [sourceRows, duplicateRows, eligibleRows, universeRows, filteredRows, usedRows, boundaryRows, temporalRows] = await Promise.all([
  ...
  const source = Number(src.source_rows || 0);
  ...
  const reconciliation = reconcileUniverse({
    source, eligible, universe, filtered, used,
    excludedIneligible, excludedByUniverse, excludedByFilter, excludedInvalid,
  });
  if (reconciliation.status === 'FAIL') {
    return fail('UNIVERSE_RECONCILIATION_FAILURE', 'universe reconciliation failed', input);
  }

  const t = temporalRows[0] || {};
  const temporal = {
    non_null:Number(t.non_null || 0),
    parsed:Number(t.parsed || 0),
    invalid:Number(t.invalid || 0),
    null:Number(t.null || 0),
  };
  const boundary = boundaryRows[0]?.boundary ?? null;
  const checks = [
    { name:'VIN Universe Audit', status:'PASS', details:{ source_rows:source, null_vin:Number(src.null_vin || 0), blank_vin:Number(src.blank_vin || 0), duplicate_vin:0, eligible_vin:eligible } },
  ];
  ...
  return {
    ok:true, status, cube:{ name:VIN_CUBE.name, version:VIN_CUBE.version },
    operation:'TEMPORAL_BOUNDARY', query:input,
    result:{ boundary, boundary_type:input.boundary, time_role:input.time.role, grain:input.time.grain },
    ...
    lineage:input.options?.include_lineage === false ? undefined : {
      ...
      normalizations:['VIN:TRIM','text:TRIM+whitespace+UPPER'],
      identity_masters:[],
    },
  };
}
...
export async function run(input = {}) {
  ...
  const sql = db();
  if ((input.operation || 'AGGREGATE') === 'TEMPORAL_BOUNDARY') {
    return runTemporalBoundary(sql, input, plan);
  }
  const [sourceAuditRows, duplicateRows, eligibleRows, universeRows, filteredRows, usedRows, groupCountRows, groupedTotalRows, resultRows, temporalRows] = await Promise.all([
    ...
    sql.query(plan.eligibleCount),
    sql.query(plan.universeCount),
    sql.query(plan.filteredCount, plan.filterValues),
    sql.query(plan.usedCount, plan.filterValues),
