...
export function reconcileUniverse({
  ...
  const sourceOk = source === eligible + excludedIneligible;
  const eligibleOk = eligible === universe + excludedByUniverse;
  const universeOk = universe === filtered + excludedByFilter;
  const filteredOk = filtered === used + excludedInvalid;
  return {
    status: sourceOk && eligibleOk && universeOk && filteredOk ? 'PASS' : 'FAIL',
    equations: {
      source: sourceOk,
      eligible: eligibleOk,
      universe: universeOk,
