...
export function applyMarketOrigin(context, requestedGroup = null) {
  ...
  const targetMismatch = Boolean(requestedGroup)
    && targets.some((row) => row.marketOrigin.originGroup !== requestedGroup);
  const warnings = [...context.warnings];
  if (requestedGroup && unknownUnits) warnings.push('MARKET_ORIGIN_UNKNOWN_PRESENT');
  if (requestedGroup && targetUnknown) warnings.push('TARGET_MARKET_ORIGIN_UNKNOWN');
  if (targetMismatch) warnings.push('TARGET_ORIGIN_GROUP_MISMATCH');
  const filteredOk = !targetUnknown && !targetMismatch && unknownUnits === 0;
  return {
    ...context,
    ...
    validation: {
      ...context.validation,
      market_origin_complete: unknownUnits === 0,
      ok: requestedGroup ? context.validation.ok && filteredOk : context.validation.ok,
    },
